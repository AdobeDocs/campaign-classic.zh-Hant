...
* **解決任何移轉後問題的計畫為何？**
  ...
  在生產系統移轉之前，將會進行廣泛的測試。 但如果發生任何問題， [Adobe客戶服務](https://experienceleague.adobe.com/?support-solution=Campaign#support) 仍會是主要聯絡視窗。 Adobe已建立專家團隊，在需要時提供進階支援。
...
* **我們將如何處理新增至允許清單的連線埠，以進行SFTP/FTP存取？**
...
* **移轉是否會產生新的追蹤、資源和Web應用程式URL？**

  否，我們將保留現有的URL。

* **子網域會從Neolane.net變更為campaign.adobe.com嗎？**

  兩者 `neolane.net` 和 `campaign.adobe.com` 將在移轉後就位。 簡單起見：我們將重新導向neolane.net至公用雲端(AWS)中的新執行個體，因此客戶不需變更。
...
* **移轉是否會影響使用Genesis聯結器的連線至Adobe Analytics？ 如何將Campaign IP位址新增至Adobe Analytics端的允許清單？**
...
* **存取特定副檔名是否有任何風險？**
  ...
  客戶可能想要允許將字型檔案、outlook會議檔案載入公用資源資料夾。 此設定目前完成 `config-<instance>.xml` 檔案。 這將會與組態檔一起複製。

* **新伺服器上的時區是否正在變更？ 客戶能保持目前的時區嗎？**

  它可能會根據新伺服器的位置而變更。 不過，客戶將可保留其目前的時區。

  [瞭解更多](../../workflow/using/managing-time-zones.md) 關於Adobe Campaign Classic v7中的時區管理
...
* **如果無法建立SFTP連線，那麼有什麼變通辦法/計畫可讓客戶繼續營運？**
...
* **透過IMS整合，它可以在新執行個體中正常運作還是需要任何額外的設定更新？**
