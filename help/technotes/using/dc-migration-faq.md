...
您會在下方找到關於此專案的一組常見問題、對您的Campaign環境的影響，以及其他有用的資源。
...
* **舊式資料中心和公共雲之間的套件大小是否有任何差異？**
   ...
   我們正在Public Cloud(AWS)中根據當前資料庫大小、磁碟大小等使用新的包定義進行配置。 例如，如果客戶在舊式資料中心有一個應用程式伺服器，則他們可以根據包定義在公共雲(AWS)中可以有兩個應用程式伺服器。
...
* **如何處理新增至允許清單的埠，以便存取SFTP/FTP?**

   SFTP設定（公開金鑰+允許清單上的IP）也將從舊版資料中心移至公開雲端(AWS)。 預期客戶不會採取任何動作。
...
* **我們使用的委派子網域對溝通方式有何影響？**
   ...
   用於行銷通訊的子網域將維持不變。 不過，視實施而定，用戶端可能需要執行下列動作：
...
* **移轉是否會影響與其他Adobe解決方案(AEM、Target等)的連線？**
   ...
   整合是在允許清單上宣告之IP位址和Web服務帳戶設定的組合。 這將由Adobe客戶服務負責並擁有。

   允許清單上會有IP地址，當應用程式伺服器IP將更改時，外部解決方案中將需要這些地址。 將提供此資訊。 整合的其他部分是以IMS為基礎，應能如常運作。
