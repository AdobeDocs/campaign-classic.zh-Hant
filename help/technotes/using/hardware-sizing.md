---
product: campaign
title: Campaign Classic v7的硬體調整建議
description: Campaign Classic v7的硬體調整建議
feature: Technote
...
>[!CAUTION]
>
>本文僅作為一般範例指南提供。 在開始Campaign專案之前，您必須與您的Adobe Campaign客戶成功經理互動，以測量部署的確切規模。 **完成之前，請**&#x200B;不要取得或部署任何基礎結構或硬體。

本檔案提供在內部部署資料中心或虛擬雲端環境進行Adobe Campaign Classic v7部署的一般建議。 此型別的部署（稱為&#x200B;**混合**&#x200B;或&#x200B;**中間來源**）會將Campaign行銷執行個體和行銷資料庫置於您的營運控制之下，同時使用Adobe雲端傳訊服務來傳送電子郵件、簡訊或SMPP訊息，並收集電子郵件開啟、彈回和點選追蹤資料。

行銷執行個體是Adobe Campaign架構的一部分，可驅動所有行銷活動，並儲存行銷活動傳回的所有收件者資料和分析資料。 行銷執行個體是一組執行Adobe Campaign服務的內部部署伺服器，以及一個關聯式資料庫。

>[!CAUTION]
>
>如果您使用完全託管的Adobe Campaign執行個體（部署在Adobe Cloud Services中），則本檔案中的資訊不適用。

軟體相容性在[相容性矩陣](../../rn/using/compatibility-matrix.md)中有詳細說明。
...
1. [中等大小](#scenario-1) — 系統中有500萬個使用中的收件者
1. [大型](#scenario-2) — 系統中有2,000萬個使用中的收件者
1. [Enterprise](#scenario-3) - 5,000萬名使用中的收件者，包含交易式訊息
...
行銷執行個體中的應用程式伺服器需要足夠的CPU和記憶體，才能執行工作流程並回應SOAP API呼叫，包括Campaign Console使用者的請求。 對於使用具有複雜選件規則的對外互動、執行自訂Javascript的工作流程以及高流量層級的網頁應用程式的工作流程，CPU需求可能相當重要。
...
基於安全性和可用性，Adobe建議將網際網路流量與業務使用者產生的流量分開。 因此，圖表包含兩組伺服器：網頁伺服器（面對網際網路的Web1和Web2）以及應用程式伺服器（業務流程App1和App2）。

對於商業電子郵件寄件者，法律規定必須有功能性的選擇退出網頁。 針對容錯移轉情況，Adobe建議在每一群組伺服器上安裝備援電腦。 如果Adobe Campaign託管選擇退出頁面，情況尤其如此。

### 反向代理

Campaign架構會使用HTTP上的SSL (HTTPS)來在行銷執行個體與Adobe雲端傳訊之間通訊，以強化高安全性。 在「非軍事區域」(DMZ)子網路中使用反向代理來隔離行銷執行個體伺服器和資料庫，確保安全性、可靠性和可用性。

### 負載平衡器

應用程式伺服器的負載平衡器是以主動/被動設定來設定，且HTTPS會在Proxy終止。 Web伺服器的負載平衡器是以主動/主動設定來設定，且HTTPS會在Proxy終止。

Adobe提供可在部署環境中轉送至Adobe Campaign伺服器的URL路徑專屬清單。

### 架構

無論磁碟區為何，一般架構幾乎完全相同。 安全性和高可用性需求要求至少有四台伺服器；若未使用WebApps，則有兩台伺服器。 組態的差異主要在於硬體組態，例如CPU核心和記憶體。
...
針對這些磁碟區，一對Adobe Campaign應用程式伺服器系統為Adobe Campaign使用者端使用者和工作流程執行提供所有功能。 對於500萬使用中的收件者及此電子郵件量，應用程式伺服器工作負荷不會耗費在CPU或I/O上；大部分的壓力都集中在資料庫上。
...
**3Ghz+四核心CPU、8-GB RAM、RAID 1或10、2 x 80-GB固態硬碟**
...
**3Ghz+ 4核心CPU、16GB RAM、RAID 1或10、128GB的SSD最小容量**
...
另外也考慮到由於每天處理的資訊量大，資料庫伺服器的IOPS非常關鍵。 例如，在高峰日，您可以部署目標為總計500,000位收件者的行銷活動。 為了執行每個行銷活動，Adobe Campaign會將500,000筆記錄插入包含約1,200萬筆記錄的表格（傳送記錄表格）。 為了在行銷活動部署期間提供可接受的效能，Adobe建議此情境下至少60,000個4-KB的隨機讀取/寫入IOPS。
...
請參閱[案例1：中等大小部署](#scenario-1)，以取得代理程式、喜好設定中心/訂閱處理及磁碟空間使用量的更多註解。

### 資料庫

資料庫伺服器的硬體建議如下：

**3Ghz+ 8核心CPU、64GB RAM、RAID 1或10、2個320GB固態硬碟或RAID 10、640GB固態硬碟，最少**
...
支援5000萬收件者的部署基本上與[案例2](#scenario-2)中所示相同：行銷活動網頁應用程式流量會路由至Campaign網頁伺服器，所以大型行銷活動啟動後爆發的網頁流量不會影響Campaign工作流程和Client Console使用者。
...
此情境中，Adobe建議您在四部機器上安裝Adobe Campaign，如下所示：
...
**3Ghz+ 8核心CPU、96-GB RAM、RAID 1或10、1.5TB固態硬碟（最低）**
...
* **個收件者數目**
作用中的收件者需要儲存空間與資料庫緩衝區空間，因此較多的收件者通常需要資料庫伺服器上的更多記憶體和CPU容量。 收件者的儲存空間增加相對較小，但對於保留用於電子郵件促銷活動的事件追蹤資料而言，可能十分重要。

* **電子郵件行銷活動大小**
行銷活動啟動頻率會影響資料庫伺服器CPU需求。 結合直接郵寄、傳入互動和其他工作流程，電子郵件促銷活動的細分作業會對資料庫伺服器造成大量負載。

* **直接郵件頻率**
直接郵寄的頻率可能會影響資料庫伺服器CPU的需求。 結合行銷活動啟動和其他工作流程，直接郵件的分段作業會對資料庫伺服器造成大量負載。

* **SMS訊息量**
如同電子郵件行銷活動大小，SMS訊息容量不會將大量負載置於內部部署的Campaign伺服器上；負載大多位於雲端上的Adobe雲端訊息伺服器上。 SMS行銷活動（例如電子郵件和直接郵件）的區段可能會對行銷資料庫造成大量負載。 因此，SMS行銷活動的啟動頻率和區段的複雜性比SMS訊息的數量更相關。

* **資料庫結構描述複雜性**
每個作用中收件者的資料量需要儲存空間與資料庫緩衝區空間，因此較多的收件者通常需要資料庫伺服器上的更多記憶體和CPU。 複雜的結構描述也需要聯結更多表格才能進行分段，因此分段作業的執行速度會更慢，而且當資料散佈在多個表格時，會需要更多資料庫CPU和記憶體。

  資料庫伺服器記憶體的預估方法是，確保資料庫緩衝區集區足夠大，可以包含所有收件者資料，再加上執行工作流程的臨時表格，以及其他資料庫作業的利潤。

* **傳出互動使用量**
在將所有計算複雜性移交給資料庫的工作流程中，會評估批次模式下的互動規則。 資料庫上主要的成果是在一個引擎呼叫期間計算的合格優惠方案總數（目標大小X每個收件者的平均優惠方案數，之後保留N個最佳優惠方案）。 資料庫伺服器CPU的速度是效能的第一個因素。

* **傳入互動或SOAP API使用情形**
傳入互動規則和選件會在行銷資料庫中評估，需要大量的資料庫伺服器資源，特別是CPU。 大量使用傳入互動或SOAP API需要個別的網頁伺服器，以將工作負載和執行行銷活動工作流程分開。
...
* **I/O效能**
必須遵循資料庫儲存的建議的IOPS評等。 Amazon EC2等雲端服務可能無法提供所需的效能，必須仔細評估。 例如，Amazon EC2布建的SSD磁碟區目前每個的評等為20,000 IOPS。 在[Amazon檔案](https://docs.aws.amazon.com/AWSEC2/latest/UserGuide/ebs-volume-types.html))中瞭解更多，因此4磁區RAID組態的評等為80,000 IOPS，這可能還不夠。
