...
透過Campaign Classic，所有一般使用者都已能使用其Adobe ID，透過Adobe Campaign Identity Management System (IMS)，連線至Adobe使用者端主控台。 但是，使用者/密碼連線仍然可用。 Campaign v8將不再允許此行為。

此外，為了強化安全性和驗證程式，Adobe Campaign使用者端應用程式現在直接使用IMS技術帳戶權杖呼叫Campaign API。 技術運運算元的移轉作業在專用文章中有詳細說明，可在[此頁面](ims-migration.md)取得。

此變更已適用於Campaign Classic v7，且&#x200B;**強制性**&#x200B;才能移至Campaign v8。
...
在開始移轉程式之前，您必須聯絡您的Adobe轉換經理(適用於Managed Services客戶)，或Adobe客戶服務（適用於其他託管客戶），讓Adobe技術團隊可以將您現有的操作員群組及已命名的許可權移轉至Adobe Identity Management System (IMS)。
...
1. 請與您的Adobe代表/客戶服務合作，確保Adobe有日期，可供非技術使用者（操作員）和產品設定檔執行自動移轉。 此步驟需要一小時的時段，您的任何服務都不需要停機時間。
...
移轉一旦完成，請聯絡您的Adobe轉換經理(適用於Managed Services使用者)，或聯絡Adobe客戶服務（適用於託管客戶）。 接著Adobe會將移轉標籤為完成。 接著您的環境就會受到保護並標準化。
...
對於託管和Managed Services客戶，為了完成移轉（移轉使用者和產品設定檔），Adobe需要一小時的時間，您的任何執行個體（工作流程等）都不需要停機時間。
...
如果您的所有使用者（Campaign運運算元）都在IMS上，您仍需聯絡Adobe代表/客戶支援以計畫產品設定檔移轉。 您還需要移轉可能在自訂程式碼中使用的技術使用者。 在[本頁](ims-migration.md)中瞭解更多。
