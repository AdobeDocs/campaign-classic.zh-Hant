...
1. 如果資料屬於查詢的篩選維度，只需在可用欄位清單中選取該維度，即可在輸出欄中顯示。
   ...
   * 根據從目標母體取得的資料或匯總（上個月的待定購買數、收據的平均金額等）計算的欄位。 例如，請前往 [選取資料](targeting-data.md#selecting-data).
...
如需篩選維度的詳細資訊，請參閱 [本節](building-a-workflow.md#targeting-and-filtering-dimensions).
