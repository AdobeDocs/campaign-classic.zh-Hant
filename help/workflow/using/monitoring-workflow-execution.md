...
也提供了如何建立工作流程，讓您監控「暫停」、「停止」或「有錯誤」之工作流程集的狀態的使用案例，其位於 [本節](supervising-workflows.md#supervising-workflows).

此外，執行個體的管理員可使用 **稽核軌跡** 若要檢查活動和上次對工作流程完成的修改，請查看工作流程的狀態。 有關詳細資訊，請參閱 [Campaign Classicv7生產指南](../../production/using/audit-trail.md).

監控不同促銷活動程式的其他方式，請參閱 [Campaign Classicv7生產指南](../../production/using/monitoring-guidelines.md).
...
* 僅查看連結到給定任務的那些消息。 若要這麼做，請選取 **[!UICONTROL Tasks]** ，然後在圖表中選取活動以限制清單。 按兩下任務以顯示資訊；視窗中的最後一個索引標籤包含記錄檔。
   ...
   此 **[!UICONTROL Details...]** 按鈕可讓您顯示有關活動執行的所有其他資訊。 例如，您可以檢視驗證運算子，以及在核准期間輸入的註解（若適用），如下列範例所示：
...
不會自動清除工作流歷史記錄：預設會保留所有訊息。 歷史記錄可透過 **[!UICONTROL File > Actions]** 或按一下 **[!UICONTROL Actions]** 按鈕。 選取 **[!UICONTROL Purge history]**。中可用的選項 **[!UICONTROL Actions]** 功能表在 [動作工具列](starting-a-workflow.md) 區段。
...
您可以設定工作流程，使其在出現錯誤時不會暫停並繼續執行。要執行此操作，請編輯工作流 **[!UICONTROL Properties]** 和 **[!UICONTROL Error management]** 區段，選取 **[!UICONTROL Ignore]** 選項 **[!UICONTROL In case of error]** 欄位。 然後，您可以指定在程序暫停之前可以忽略的連續錯誤數。
...
工作流程使用大量工作表格，耗用空間，若不進行維護，最終會使整個平台變慢。 有關資料庫維護的詳細資訊，請參閱 [節](../../production/using/tables-to-maintain.md) .

此 **資料庫清理** 可透過 **管理>生產>技術工作流程** 節點，可讓您刪除過時的資料，以避免資料庫呈指數增長。 工作流程會自動觸發，使用者不需干預。 請參閱 [Campaign Classicv7生產指南](../../production/using/database-cleanup-workflow.md).

您也可以建立特定技術工作流程，以清除不必要的資料佔用空間。 請參閱 [Campaign Classicv7生產指南](../../production/using/application-objects.md) 和這個 [節](#purging-the-logs).
...
* **[!UICONTROL Start Pending]**：列出operationMgt程式等待啟動的所有工作流程。 此檢視可搭配 **行銷活動** 僅包。 深入了解 [Campaign Classicv7安裝指南](../../installation/using/installing-campaign-standard-packages.md))。
...
1. 將資料夾添加到樹狀結構後，請確保將其配置為視圖，以便它顯示所有工作流（無論其原始資料夾是什麼）。有關如何配置視圖的詳細資訊，請參閱 [本節](../../platform/using/access-management-folders.md).
