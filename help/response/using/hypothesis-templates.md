...
1. 按一下 **[!UICONTROL New]** 或在範本清單中按一下滑鼠右鍵，然後選擇 **[!UICONTROL New]** ，位於下拉式清單中。
1. 輸入假設標籤。
1. 指定範本是預期用於優惠或傳遞的假設，透過 **[!UICONTROL Hypothesis type]**.
1. 的 **[!UICONTROL Delivery]** 輸入範本，指定是否應在控制組中進行測量。 [了解更多](#properties-of-a-hypothesis-template)
1. 的 **[!UICONTROL Delivery]** 輸入範本，您可以選擇特定管道，或決定使用將範本套用至Adobe Campaign中的所有可用管道。 **[!UICONTROL Channel]** 下拉式清單。 [了解更多](#properties-of-a-hypothesis-template)
1. 選取 **[!UICONTROL Execution folder]** 其中您想要建立並自動執行將從此範本建立的假設。
...
* **[!UICONTROL Channel]**：您可以選取特定管道，或選取「 」，讓假設範本可用於Adobe Campaign主控台中的所有管道 **[!UICONTROL All channels]** ，位於下拉式清單中。 如果您設定特定頻道的範本，這可讓您在建立假設時自動篩選每個頻道的傳送。 [了解更多](creating-hypotheses.md)
...
* **[!UICONTROL Priority]**：套用至假設的層級，以在同時執行時分隔假設計算順序。

  ![](assets/response_exec_settings_003.png)

* **[!UICONTROL Automatic execution]**：如有需要，可讓您排程假設重新計算（例如，如果您想要定期更新指標，直到傳送結束為止）。
...
* **[!UICONTROL Log SQL queries in journal]**：此函式為專家使用者保留。 它可讓您在測量假設稽核中新增索引標籤，以顯示SQL查詢。 如此一來，在模擬完成時若發生錯誤，即可偵測可能的故障。
...
* **[!UICONTROL Schema (reaction log storage)]**：選取收件者反應表格。 Adobe Campaign中的現成表格為 **NmsRemaMatchRcp**.
...
* **[!UICONTROL Transaction date]**：此欄位是選用欄位，但建議您使用，因為它可讓您定義假設計算的範圍。
* **[!UICONTROL Measurement period]**：可讓您設定執行假設及復原購買明細行的開始和結束日期。
...
定義交易表格和假設將關注的欄位後，您可以使用篩選器指定目標交易和傳送，以縮小假設的範圍。 您也可以使用JavaScript指令碼來明確指向交易表格中所參照的產品。
...
* **依收件者篩選**：在 **[!UICONTROL Scope]** 標籤，您可以將假設限製為連結至訊息的任何資訊（傳遞、收件者、電子郵件地址、服務等）：
...
在此範例中，我們將針對直接郵件型別傳遞建立假設範本。 交易表(**購買** 在我們的範例中)，這些假設將以包含連結至文章或產品的購買明細行為基礎。 我們想要設定模型，以建立購買表格中文章或產品的假設。
...
1. 選取中的採購明細行 **[!UICONTROL Querying schema]** 欄位。
...
1. 在 **[!UICONTROL Scope]** 標籤，按一下 **[!UICONTROL Edit query]** 以建立假設的篩選。
