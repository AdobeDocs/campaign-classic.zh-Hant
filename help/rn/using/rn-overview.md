...
| [!BADGE 有限可用性]{type=Informative} | 僅限隨選部署。 |
...
>[!NOTE]
>
> 每個建置都是累積性的：除了本身的變更外，也包含同一版本先前建置中提供的所有功能、改善和修正。若要取得先前修正的效益，您不需要安裝每個中間建置：安裝最新建置即可讓您取得先前建置中包含的所有內容。
...
為保護託管環境的完整性，Adobe 保留隨時套用關鍵安全性修補程式且不事先通知的權利，以盡快修補漏洞。這些修補程式會在不中斷服務的情況下部署。此做法符合 Adobe 的安全性治理標準；根據這些標準，修補重大漏洞的急迫性優先於事先通知。
