  ...
  <tr> 
   <td> <p>CCPA is the State of California’s new privacy law that harmonizes and modernizes data protection requirements going into effect on Jan 01, 2020. CCPA適用於Adobe Campaign客戶，這些客戶持有位於加利福尼亞的資料主題資料。</p>
    ...
    <ul>
      ...
      <li>您可以跟蹤消費者是否已選擇出售個人資訊。 為此，需要擴展「配置式」表並添加 <strong>CCPA的退出選項</strong> 的子菜單。 <a href="https://helpx.adobe.com/campaign/kb/acc-privacy.html#ccpa">閱讀全文</a></li></td> 
  </tr> 
 </tbody> 
</table>
...
* 推送通知的記憶體消耗優化。
* 對於效能和儲存優化， **登錄.log** 檔案已增強。 現在，該檔案被拆分為多個檔案，每天一個，最多保留365個檔案。 [閱讀全文](../../production/using/log-files.md)
* MicrosoftDynamics CRM外部帳戶現在可以使用密碼憑據（密碼+用戶名）或證書（私鑰）進行配置。 [閱讀全文](../../installation/using/external-accounts.md#microsoft-dynamics-crm-external-account)
...
* 新建 [市場活動選項](../../installation/using/configuring-campaign-options.md) 已添加：
   ...
   * 的 **WdbcOptions_TempDbName** 選項，用於為MicrosoftSQL Server上的工作表配置單獨的資料庫。 這可優化備份和複製。 [閱讀全文](../../production/using/rdbms-specific-recommendations.md#microsoft-sql-server)
   * 的 **XtkCleanup_NoStats** 已對PostgreSQL的選項進行了增強，以更好地控制資料庫清理工作流的儲存優化步驟的行為。 [閱讀全文](../../production/using/database-cleanup-workflow.md#statistics-update)
* 已將帳戶鎖定機制添加到 **登錄()** API。 它防止在指定時間範圍內連續嘗試一定數量的登錄失敗後再嘗試登錄。
* 新 **最大個性化運行時間** 交付屬性中的選項允許您定義個性化運行時間的超時期間，以防止個性化階段運行過長。 [閱讀全文](../../delivery/using/personalization-fields.md#timing-out-personalization)
* 的 **ftp協定** 已添加選項，以允許您對SFTP連接使用代理配置。 [閱讀全文](../../installation/using/file-res-management.md)
* 新支援對SFTP外部伺服器的代理訪問，用於內部環境。
* 已添加特定的保護欄，以防止安裝與市場活動實例不相容的包。 [閱讀全文](../../installation/using/installing-campaign-standard-packages.md)
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   <td> 相容性矩陣更新<br /> </td> 
   <td> 通過此新版本，Adobe Campaign現在支援以下資料庫系統。 請參閱 <a href="https://helpx.adobe.com/campaign/kb/compatibility-matrix.html">相容性清單</a>。<br /> 
    <ul> 
     <li> <p>Oracle18c</p> </li> 
...
* 出於安全原因，在使用 **[!UICONTROL Pre-process the file]** 的 **[!UICONTROL Data loading (file)]** 工作流活動。 現在可以使用下拉清單，您可以從3個選項中選擇： **[!UICONTROL None]**。 **[!UICONTROL Decompression]** (zcat)或 **[!UICONTROL Decrypt]** (gpg) 已添加XtkSecurity_Disable_Preproc安全標誌。 對於新客戶，此選項將設定為0。 對於現有客戶，此選項將按策略級設定為1以保留先前的行為。 請參閱此 [節](../../workflow/using/data-loading--file-.md)。
...
* 密件抄送存檔：BCC存檔的預設參數已更改，以提高存檔速度。 [閱讀全文](../../installation/using/email-archiving.md#parameters)
