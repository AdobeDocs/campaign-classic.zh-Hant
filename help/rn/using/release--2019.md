---
product: campaign
title: Campaign Classic2019版本
description: 深入了解Campaign Classic2019版本
 ...
 <tbody> 
  <tr> 
   <td> <p>CCPA是加州新推出的隱私權法，協調2020年1月1日起生效的資料保護要求並以現代化方式規範資料保護要求。 CCPA適用於為居住在加州的資料主體保管資料的Adobe Campaign客戶。</p>
    ...
    <ul>
      <li>存取權與刪除權：我們善用針對GDPR新增的功能。 <a href="https://helpx.adobe.com/campaign/kb/acc-privacy.html#righttoaccess">閱讀全文</a></li>
      <li>您可以追蹤消費者是否選擇退出個人資訊銷售。 為此，您需要擴充「設定檔」表格並新增 <strong>選擇退出CCPA</strong> 欄位。 <a href="https://helpx.adobe.com/campaign/kb/acc-privacy.html#ccpa">閱讀全文</a></li></td> 
  </tr> 
 </tbody> 
</table>
...
* 推播通知的記憶體耗用最佳化。
* 為實現效能和儲存優化， **logins.log** 檔案已增強。 檔案現在會分割為多個檔案，每天最多可保留365個檔案。 [閱讀全文](../../production/using/log-files.md)
* Microsoft Dynamics CRM外部帳戶現在可使用密碼憑證（密碼+使用者名稱）或憑證（私密金鑰）來設定。 [閱讀全文](../../installation/using/external-accounts.md#microsoft-dynamics-crm-external-account)
...
* 新增 [促銷活動選項](../../installation/using/configuring-campaign-options.md) 已新增：
   ...
   * 此 **WdbcOptions_TempDbName** 選項可讓您為Microsoft SQL Server上的工作表配置單獨的資料庫。 這樣可優化備份和複製。 [閱讀全文](../../production/using/rdbms-specific-recommendations.md#microsoft-sql-server)
   * 此 **XtkCleanup_NoStats** 已增強PostgreSQL的選項，以更好地控制資料庫清理工作流的儲存優化步驟的行為。 [閱讀全文](../../production/using/database-cleanup-workflow.md#statistics-update)
* 帳戶鎖定機制已新增至 **logon()** API。 它可防止在指定時間範圍內連續嘗試某些次數的失敗登入之後，再嘗試任何登入。
* 新 **最大個人化執行時間** 「傳送」屬性中的「 」選項可讓您定義個人化執行時間的逾時期間，以防止個人化階段執行太久。 [閱讀全文](../../delivery/using/personalization-fields.md#timing-out-personalization)
* 此 **ftp協定** 已新增選項，可讓您使用SFTP連線的Proxy設定。 [閱讀全文](../../installation/using/file-res-management.md)
* 針對內部部署環境，全新支援代理存取SFTP外部伺服器。
* 已新增特定護欄，以防止安裝與Campaign執行個體不相容的套件。 [閱讀全文](../../installation/using/installing-campaign-standard-packages.md)
...
請確定您使用最新Campaign相容性矩陣所列之任何系統的支援版本。 [閱讀全文](https://helpx.adobe.com/tw/campaign/kb/compatibility-matrix.html)
...
* 有效期限 — XtkNewId序列使用優化：最耗用的表格已從xtkNewId序列移至專用序列。 [閱讀全文](https://helpx.adobe.com/campaign/kb/sequence_auto_generation.html#Switchtoadedicatedsequence)
...
* 密件副本歸檔：已變更BCC封存的預設參數，以提高封存速度。 [閱讀全文](../../installation/using/email-archiving.md#parameters)
