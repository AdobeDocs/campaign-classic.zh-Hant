...
您可以選取下列項目來設定此報表的內容：要顯示的分數圖表、時間篩選（自動作啟動以來，在啟動後的前6小時，等等） 和資料顯示模式（依標籤、依URL、依類別），如需詳細資訊，請參閱 [本頁](../../delivery/using/monitoring-a-delivery.md)。 按一下 **[!UICONTROL Refresh]** 以確認您的選取。
