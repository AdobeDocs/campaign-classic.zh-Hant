...
1. 前往宇宙 **[!UICONTROL Campaigns]** 並按一下連 **[!UICONTROL Delivery]** 結以顯示傳送清單。
...
>[!NOTE]
>
>此報告中顯示的域名在立方級別使用的明細清單中定義。 若要變更、新增或移除預設網域，請編輯 **[!UICONTROL Domains]** 項目清單並修改值和別名。 如需詳細資訊，請參閱[本章節](../../platform/using/managing-enumerations.md)。類 **[!UICONTROL Others]** 別包含不屬於項目清單任何值的網域名稱。
...
您可以選取下列項目來設定此報表的內容： 要顯示的分數圖表、時間篩選（自動作啟動以來，在啟動後的前6小時，等等） 和資料顯示模式（依標籤、依URL、依類別），如需詳細資訊，請參閱 [本頁](../../delivery/using/monitoring-a-delivery.md)。 Click **[!UICONTROL Refresh]** to confirm your selection.
...
* **[!UICONTROL Messages rejected by the rule]** : 套用類型學規則時，分析期間忽略的地址數： 地址丟失、隔離、添加到塊清單等。 有關類型學規則的詳細資訊，請參閱本 [頁](../../delivery/using/steps-validating-the-delivery.md#validation-process-with-typologies)。
...
它可讓您追蹤傳送的行銷影響。 您可以變更時標（1小時、3小時或24小時檢視等），來設定值的顯示方式。 Click **[!UICONTROL Refresh]** to confirm your selection.
