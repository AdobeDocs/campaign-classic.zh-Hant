...
若要建立描述性分析報表，請使用專用精靈。 設定取決於要分析的資料和所需的演算。
...
若要分析標準收件者以外的表格(**nms：recipient**)，按一下 **[!UICONTROL Advanced settings...]** 連結精靈的最後一個階段，並選取符合您設定的表格，在此案例中為 **cus：individual**：

![](assets/reporting_descriptive_other_schema.png)

如果您想要針對部分資料產生統計資料，可以定義篩選器：若要這麼做，請按一下 **[!UICONTROL Advanced settings...]** 連結並定義要套用的篩選器，如下所示：
...
您可以透過不同內容使用描述性分析精靈：清單、工作流程轉變、一個或多個傳送、選擇收件者等。
...
* 針對一組 **收件者**，選取要分析的收件者，然後按一下右鍵並選取 **[!UICONTROL Actions > Explore...]**，如上所示。 如果篩選器套用至收件者清單，則只會分析其內容。

   若要選取資料夾或目前篩選器中的所有收件者，請使用CTRL+A快速鍵。 這表示即使未顯示的收件者也會被選取。

   有關收件者描述性分析的範例，請參閱： [質化資料分析](../../reporting/using/use-cases.md#qualitative-data-analysis).

* 在的內容中 **工作流程**，將游標放在指向收件者表格的轉變上，按一下滑鼠右鍵並選取 **[!UICONTROL Analyze target]**. 如需詳細資訊，請參閱中的範例 [在工作流程中分析轉變目標](../../reporting/using/use-cases.md#analyzing-a-transition-target-in-a-workflow).
* 對象 **清單**，選取一或多個清單並套用與收件者相同的程式。
...
## 設定定性發佈範本 {#configuring-the-qualitative-distribution-template}

此 **[!UICONTROL Qualitative distribution]** 範本可讓您建立所有資料型別（例如公司名稱、電子郵件網域）的統計資料。

針對透過建立的報告可用的設定選項 **[!UICONTROL Qualitative distribution]** 範本的詳細資訊，請參閱 [在表格中顯示資料](#displaying-data-in-the-table). 完整的範例詳見 [分析母體](../../reporting/using/use-cases.md#analyzing-a-population).
...
>[!NOTE]
>
>當計算涉及的欄位使用彙總計算時，請檢查 **[!UICONTROL The data is already aggregated]** 提升效能。
...
* **[!UICONTROL Auto]** ：此選項可讓您顯示n個最常顯示的值。 會自動計算變數，每個變數代表相較於回收桶數的變數百分比。 對於數值，Adobe Campaign會自動產生n個類別以將資料排序到。
* **[!UICONTROL Manual]** ：此選項的運作方式與 **[!UICONTROL Auto]** 選項，但您可以手動設定這些值。 若要這麼做，請按一下 **[!UICONTROL Add]** 按鈕。
...
* **[!UICONTROL Modulo]** ：可讓您建立數值群組。 例如，值為10的模可讓您建立十乘十的值間隔。
...
您可以顯示或隱藏統計資料和小計的名稱，並選擇統計資料的方向。 有關詳細資訊，請參閱 [分析報表顯示設定](../../reporting/using/processing-a-report.md#analysis-report-display-settings).

### 在圖表中顯示資料 {#displaying-data-in-the-chart}

在描述性分析精靈的第一步，您可以選擇只以圖表形式顯示資料，而不使用表格。 在此情況下，設定圖形時必須選取變數。 您必須先選取要顯示的變數數目，然後從相關資料庫中選取欄位。
...
>[!NOTE]
>
>您可以同時在圖表和表格中顯示變數。 若要這麼做，請在 **[!UICONTROL Table configuration]** 視窗。 按一下 **[!UICONTROL Next]** 並在「圖表設定」視窗中選取圖表型別。 如果在表格中定義了子維度，則它們不會顯示在圖表中。
...
描述性分析精靈可讓您計算資料的幾種統計資料。 依預設，僅會設定一個簡單計數。
...
* **[!UICONTROL Row percentage distribution]** 若要計算欄中的值與列中的值的比率（僅適用於表格），
* **[!UICONTROL Column percentage distribution]** 若要計算列中值與欄中值的比率（僅適用於表格），
...
* **[!UICONTROL Calculated field]** 建立個人化運運算元（僅適用於表格）。 此 **[!UICONTROL User function]** 欄位可讓您輸入要套用至資料的計算。
   ...
   若要在表格中顯示上述資訊，您必須建立計算欄位，以儲存每位客戶的平均購買量。
   ...
   1. 此統計資料不會顯示在表格中。 您需要取消勾選 **[!UICONTROL Display in the table]** 的選項 **[!UICONTROL Advanced]** 標籤。
...
您可以變更結果的版面。 若要這麼做，請以滑鼠右鍵按一下相關的變數，然後從快捷選單中選取輸入。
...
此 **[!UICONTROL Quantitative distribution]** 可讓您針對可測量或計數的資料（例如發票金額、收件者的年齡）產生統計資料的範本。
...
依預設，Adobe Campaign會針對選取的資料提供一系列要計算的統計資料。 您可以視需要變更此清單、新增或刪除統計資料。
...
* **[!UICONTROL Sum]** 若要計算數值欄位中值的總和，
* **[!UICONTROL Standard deviation]** 以計算傳回的值在平均值周圍的分佈方式。
* **[!UICONTROL Number of missing values]** 計算未定義值的數值欄位數目。
* **[!UICONTROL Decile distribution]** 分配傳回的值，使得每個都代表數值欄位中值的1/10。
* **[!UICONTROL Custom distribution]** 根據使用者定義的臨界值來分配傳回的值。
   ...
   若要對報表進行變更，請參閱 [處理報告](../../reporting/using/processing-a-report.md).
