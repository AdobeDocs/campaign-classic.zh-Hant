...
>[!NOTE]
>
>如果您的執行個體托管在AWS上，您也可以使用Campaign Classic監控每小時傳送的傳送數量 [控制面板](https://experienceleague.adobe.com/docs/control-panel/using/sftp-management/sftp-storage-management.html). 若要檢查您的執行個體是否託管在 AWS 上，請按照[本頁面](https://experienceleague.adobe.com/docs/control-panel/using/faq.html)詳述的步驟操作。
>
>所有管理員使用者都可存取控制面板。 授予使用者管理員存取權限的步驟已詳載於[本頁](https://experienceleague.adobe.com/docs/control-panel/using/discover-control-panel/managing-permissions.html?lang=zh-Hant#discover-control-panel)中。
>
>請注意，您的執行個體必須升級為 [Gold Standard](../../rn/using/gold-standard.md) 建置或 [最新GA版本編號(21.1.3)](../../rn/using/latest-release.md). 在[本章節](../../platform/using/launching-adobe-campaign.md#getting-your-campaign-version)中瞭解如何確認您的版本。 
...
* **[!UICONTROL Opens]** :已開啟的郵件總數。 不會考慮文字格式的電子郵件。 如需追蹤開啟的詳細資訊，請參閱 [追蹤開啟](../../reporting/using/indicator-calculation.md#tracking-opens-).
* **[!UICONTROL Clicks]** :傳遞中連結的點按總次數。 取消訂閱連結和鏡像頁面的點按次數並未納入考量。
* **[!UICONTROL Transactions]** :接收消息後的事務總數。 為了考慮事務，必須將事務類型Web追蹤標籤插入到匹配的網頁中。 網路追蹤設定如下所示： [本節](../../configuration/using/about-web-tracking.md).
...
>[!NOTE]
>
>此報告中顯示的域名在多維資料集級別定義。 若要變更這些值，請編輯 **[!UICONTROL Delivery logs (broadlogrcp)]** 立方體。 如需詳細資訊，請參閱[本章節](../../reporting/using/ac-cubes.md)。此 **[!UICONTROL Others]** 類別包含不屬於特定類的域名。
...
病毒式行銷可讓傳遞收件者與其聯絡網路共用資訊：他們可以將連結新增至其設定檔(Facebook、Twitter等) 或者給朋友發個資訊。 傳送內會追蹤每個共用和每個對共用資訊的存取。 如需病毒式行銷的詳細資訊，請參閱 [本節](../../delivery/using/viral-and-social-marketing.md).
...
在 **[!UICONTROL Shares]** 欄，我們有下列指標：

* **[!UICONTROL No. of sharing activities]** :每個社交網路上共用的訊息總數。 此值等於相符項目圖示的點按總次數 **[!UICONTROL Links for sharing to social networks]** 個人化區塊。
* **[!UICONTROL Breakdown]** :此比率代表每個社交網路的分享次數，與分享總數的關係。
* **[!UICONTROL Sharing rate]** :此比率代表每個社交網路分享次數的劃分，與要傳送的訊息數量相關。

在 **[!UICONTROL Opens]** 欄，我們有下列指標：

* **[!UICONTROL No. of opens]** :由轉送訊息的對象(透過 **[!UICONTROL Links for sharing to social networks]** 個人化區塊)。 此值等於鏡像頁面的顯示次數。 傳送收件者開啟的次數不會納入考量。
...
* **[!UICONTROL New contacts]** :接收透過電子郵件共用的訊息後的新訂閱數。 此值與收到透過電子郵件共用之訊息、按一下 **[!UICONTROL Subscription link]** 並填寫訂閱表格。
* **[!UICONTROL Opens]** :由已轉送訊息的人員開啟的訊息總數(透過 **[!UICONTROL Link for sharing to social networks]** 個人化區塊)。 此值等於鏡像頁面的顯示次數。 傳送收件者開啟的次數不會納入考量。
* **[!UICONTROL Sharing activities]** :透過社交網路共用的訊息總數。 此值會比對 **[!UICONTROL Links for sharing to social networks]** 個人化區塊。
