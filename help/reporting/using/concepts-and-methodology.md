...
# 多維度資料集的最佳實務{#concepts-and-methodology}



## 資料量化 {#data-binning}

量化可讓您透過根據條件分組值來簡化資料顯示。 根據您可用的資訊，您可以定義年齡群組、將電子郵件網域群組在一起、限製為列舉值、明確限制資料在專用行或欄中顯示及群組所有其他資料等。
...
在以下範例中，所有開頭為的電子郵件網域 **yahoo** (yahoo.fr、yahoo.com、yahoo.be等)，或使用 **ymail** (ymail.com、ymail.eu等) 將分組在標籤下 **YAHOO！**，以及包含的位址 **rocketmail.com** 網域。
...
* 復原最常用的值
   ...
   在以下範例中，將會顯示4個最常用的值，而其他值將會在「其他」類別中計數並分組。
...
您可以視需要建立任意數量的量測：然後選取要在表格中顯示或隱藏的量測。 有關詳細資訊，請參閱 [顯示量值](#displaying-measures).
...
* 要在報告中顯示/隱藏的資訊(請參閱 [設定顯示區](#configuring-the-display))
