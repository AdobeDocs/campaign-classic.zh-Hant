...
Adobe Campaign可讓您控制已排程、已確認及已開立商業發票的行銷成本，並使用「行銷資源管理」模組依類別加以劃分。
...
1. 定義成本計算方法
   ...
   為服務提供者定義成本結構。 另請參閱 [建立服務提供者及其成本類別](../../campaign/using/providers--stocks-and-budgets.md#creating-a-service-provider-and-its-cost-categories).

1. 定義行銷活動成本（傳遞/任務）

   傳遞和任務產生的成本會個別或全域輸入給行銷活動範本。 另請參閱 [成本與庫存的計算](../../campaign/using/marketing-campaign-deliveries.md#calculation-of-costs-and-stocks).

1. 合併

   根據任務、傳遞和行銷活動的進度狀態，將計算成本並傳遞至相應的預算。

   當行銷活動的建立足夠進階時，行銷活動預算的進度狀態可以變更為 **[!UICONTROL Specified]**. 然後，系統會自動輸入方案的已計算成本，以及促銷活動上已計算的成本。 另請參閱 [成本承諾、計算及收費](#cost-commitment--calculation-and-charging).

## 建立預算 {#creating-a-budget}

預算會透過以下方式在地圖中建立： **[!UICONTROL Campaign management > Budgets]** 節點。 此 **[!UICONTROL New]** 工具列中的按鈕可讓您建立預算。
...
* 輸入初始金額
   ...
   在相關欄位中指出配置的金額。 其他金額會自動輸入。 另請參閱 [計算金額](#calculating-amounts).
...
* 費用
   ...
   建立費用類別，將行銷活動、任務等的成本指派給此預算。 可以連結。 另請參閱 [費用類別](#expense-categories).
...
>您可以選取相關預算。
>
>有關詳細資訊，請參閱 [將預算連結至另一個預算](#linking-a-budget-to-another).
...
>[!NOTE]
>
>成本類別與結構呈列於 [建立服務提供者及其成本類別](../../campaign/using/providers--stocks-and-budgets.md#creating-a-service-provider-and-its-cost-categories).
...
可透過預算核准，對已承諾給行銷活動的金額執行檢查。 可在行銷活動中建立其他查核點樣式任務，以設定其他核准。 另請參閱 [任務型別](../../mrm/using/creating-and-managing-tasks.md#types-of-task).
...
在此行銷活動中，我們將新增先前已建立成本結構的兩個任務(請參閱 [步驟2 — 設定服務提供者並定義成本結構](#step-2---configuring-the-service-provider-and-defining-the-cost-structures))。 若要這麼做，請在行銷活動控制面板中，按一下 **[!UICONTROL Add a task]** 按鈕。 為任務命名，然後按一下 **[!UICONTROL Save]**.
