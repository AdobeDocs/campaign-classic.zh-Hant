...
上傳完成後，資源會新增至可用資源清單中。 Adobe Campaign操作者可存取該網站。 他們可以檢視(透過 **[!UICONTROL Preview]** 標籤)、製作復本以修改它，或更新伺服器上的檔案(使用 **[!UICONTROL Edit]** 標籤)。

![](assets/s_ncs_user_mkg_resource_extract.png)

按一下 **[!UICONTROL General]** 標籤以選取負責監視、追蹤及核准此資源的操作者或操作者群組。 選取檢閱者可透過以下方式完成 **[!UICONTROL Advanced parameters]** 連結。
...
此 **[!UICONTROL Tracking]** 索引標籤可讓您監視對資源執行的任何動作：核准、拒絕核准、相關註解或出版物。
...
可透過核准資源 **[!UICONTROL Approve resource]** 按鈕。
...
此 **[!UICONTROL Tracking]** 索引標籤可讓所有操作者追蹤核准流程的各個階段。
...
此 **[!UICONTROL Details]** 按鈕可讓您編輯與檢視。
...
>[!NOTE]
>
>如需傳送概要的詳細資訊，請參閱 [關聯並建構透過傳遞大網連結的資源](../../campaign/using/marketing-campaign-deliveries.md#associating-and-structuring-resources-linked-via-a-delivery-outline).
...
如有必要，您可以透過以下方式編輯所選資源： **[!UICONTROL Edit the link]** 圖示（放大鏡）選取資源後，即會顯示在資源的右側。
...
行銷資源儀表板可讓您執行常見的操作型別：新增、編輯、鎖定/解鎖、核准、發佈。 您可以建立其他型別的行銷資源，並透過Adobe Campaign樹存取進階功能。 若要這麼做，請按一下 **[!UICONTROL Explorer]** (位於Adobe Campaign首頁)。

根據預設，行銷資源儲存在 **[!UICONTROL MRM > Marketing resources]** 樹狀結構的節點。
