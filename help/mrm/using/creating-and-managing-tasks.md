...
當您顯示此檢視時，系統會自動篩選該檢視以便僅顯示 **[!UICONTROL operator tasks]**. 您也可以使用視窗上半部分的欄位來篩選任務。
...
若要建立任務，請按一下 **[!UICONTROL Tasks]** 中的連結 **[!UICONTROL Campaigns]** 標籤並選取 **[!UICONTROL Create]**.

![](assets/s_ncs_user_task_create_new.png)

至少輸入任務的名稱，然後選取它連結到的行銷活動。 您也必須指定開始和結束日期。 這三個資訊是強制性的。
...
建立任務後，即會將其新增至行銷活動排程和任務清單。 若要編輯任務，請從排程中選取該任務，或在任務概述中按一下其名稱，然後按一下 **[!UICONTROL Open]** 連結。

![](assets/s_ncs_user_task_edit_simple.png)

若要進行設定，您必須指出：

* 經理與參與者：請參閱 [經理與參與者](#manager-and-participants).
* 建立排程：請參閱 [執行排程](#execution-schedule).
* 已確認成本：請參閱 [費用與收入](#expenses-and-revenues).

也可以新增稽核者(請參閱 [檢閱者](#reviewers))和參考檔案(請參閱 [引用的檔案](#documents-referenced))。

任務生命週期顯示於 [生命週期](#life-cycle).

### 經理與參與者 {#manager-and-participants}

只有負責任務的操作員才有權關閉任務。

根據預設，Adobe Campaign運運算元建立任務時，會自動指派給他們。 若要選取其他運運算元，請使用 **[!UICONTROL Assigned to]** 欄位。

![](assets/s_ncs_user_task_edit_simple_general_tab.png)

>[!NOTE]
>
>運運算元管理在中呈現 [本節](../../platform/using/access-management.md).

您可以指定執行任務時涉及的運運算元。 這些操作員無權關閉任務。 他們只能核准指派給他們的任務。

這些範本是使用 **[!UICONTROL Resources]** 圖示加以檢視。 按一下 **[!UICONTROL Add]** 並選取相關的運運算元。

![](assets/s_ncs_user_task_add_resources.png)

按一下 **[!UICONTROL Ok]** 然後輸入使用率：這表示在任務執行期間指派給運運算元的負載。 此比率僅供參考，並以百分比表示。

例如，對於執行排程設定為10天的任務，使用率為50%的運運算元將會在此任務上動員10天的工作時間的一半。

對於每個運運算元，您可以輸入排程工作負載與實際工作負載。 這些持續時間也僅供參考。

您可以設定提醒，提醒將會在任務結束日期之前自動傳送給與任務有關的所有操作員。

您可以透過以下方式檢視Adobe Campaign運運算元設定檔 **[!UICONTROL Edit link]** 圖示。
...
除了參與者之外，您還可以定義操作員，當任務負責人關閉任務後，這些操作員將稽核任務。 若要這麼做，請按一下 **[!UICONTROL Enable task approval]** 的左下角區段中的選項 **[!UICONTROL Resources]** 視窗。 這可以是個別運運算元、一組運運算元或運運算元清單。
...
您可以在稽核者設定視窗的下半部定義任務的核准排程。 依預設，稽核者從提交日期起有三天時間來核准任務。 您可以設定提醒，提醒會在核准期限之前自動傳送給相關操作員。
...
您可以將檔案和行銷資源新增至任務(如需詳細資訊，請參閱 [管理行銷資源](../../mrm/using/managing-marketing-resources.md))。 若要這麼做，請開啟工作並按一下 **[!UICONTROL Documents]** 圖示加以檢視。

按一下 **[!UICONTROL Add]** 並選取要新增至您工作的檔案。 對行銷資源套用相同的程式。
...
>[!NOTE]
>
>如需費用與預算的詳細資訊，請參閱 [成本承諾、計算及收費](../../mrm/using/controlling-costs.md#cost-commitment--calculation-and-charging).
...
>[!NOTE]
>
>管理預算與成本的方法顯示於 [控制成本](../../mrm/using/controlling-costs.md).
