...
1. 開啟您要套用規則的傳遞。
1. 開啟傳遞屬性並存取 **[!UICONTROL Typology]** 標籤。
1. 在下拉式清單中選取型別。
...
>[!NOTE]
>
>對於篩選規則，您可以選取篩選條件的套用條件：它們可以取決於傳遞或傳遞大網。 有關詳細資訊，請參閱 [調整篩選規則](filtering-rules.md#conditioning-a-filtering-rule).
...
1. 控制規則（如果這些規則是在個人化開始時套用）。如果使用者規則（篩選/壓力/電容性）已過期，且需要重新計算，則會在此步驟中套用這些規則。
...
例如，執行順序為20的壓力規則將會在執行順序為30的壓力規則之前執行。

### 控制規則 {#control-rules}

對象 **[!UICONTROL Control]** 規則，您可以決定要在傳遞生命週期的哪個時間點套用規則（在鎖定目標之前或之後、在個人化開始時、在分析結束時）。 選取要在的下拉式清單中套用的值 **[!UICONTROL Phase]** 欄位，在 **[!UICONTROL General]** 型別規則的索引標籤。
