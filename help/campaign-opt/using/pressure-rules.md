...
1. 在行銷活動型別規則清單中，按一下 **[!UICONTROL New]** 圖示加以識別。
...
1. 視需要變更執行順序。 當多個型別規則套用為 **[!UICONTROL Typology]** 設定，系統會先套用順序較低的規則。 有關詳細資訊，請參閱 [執行順序](applying-rules.md#execution-order).
1. 在 **[!UICONTROL Calculation parameters]** 區段，定義頻率，以儲存下一個每日重新仲裁執行之後的目標定位。 有關詳細資訊，請參閱 [調整計算頻率](applying-rules.md#adjusting-calculation-frequency).
1. 按一下 **[!UICONTROL Pressure]** 標籤並選取套用型別規則的日曆期間。
   ...
   >[!NOTE]
   >
   >只有在符合以下條件時，才會考量已排程的傳遞： **[!UICONTROL Take the deliveries into account in the provisional calendar]** 選項時才會選擇此選項。 有關詳細資訊，請參閱 [設定期間](#setting-the-period).
   >
   >此選項在Campaign v8中無法使用。
...
1. 指定計算傳遞權重的方法。
   ...
   每個傳遞都有權重，即代表其優先順序等級的值：這會啟用行銷活動之間的仲裁。 權重是使用型別規則和/或其屬性中定義的公式來計算。 有關詳細資訊，請參閱 [訊息權重](#message-weight).
...
1. 此 **[!UICONTROL Typologies]** 索引標籤可讓您檢視套用此規則的促銷活動型別，或將規則連結至一或多個現有型別。 有關詳細資訊，請參閱 [套用型別](about-campaign-typologies.md#applying-typologies).
...
若要定義臨界值，您可以使用連結至目標維度的維度：例如，若要包含傳送至儲存在訪客表格中的收件者設定檔的訊息(如需訪客表格的詳細資訊，請參閱 [本節](../../surveys/using/use-case--creating-a-refer-a-friend-form.md))，或避免每週傳送超過一封訊息給相同的家庭（可能指多個電子郵件地址），此維度會在連結至收件者的維度中識別。
...
請注意，根據預設，計算臨界值時只會考慮已傳送的傳遞。 在Campaign Classicv7中，檢查 **[!UICONTROL Take the deliveries into account in the provisional calendar]** 選項，如果您也想要考量在相關期間內排程的傳遞。 在這種情況下，考慮的期間會加倍，以便整合未來傳送與先前傳送。

若要將考慮的傳送限制在2週期間，您可以：

1. 輸入 **15天** 在 **[!UICONTROL Concerned period]** 欄位：在計算時，會考慮在套用規則的傳送日期前兩週傳送的傳送，

或

1. 輸入 **7天** 在 **[!UICONTROL Period considered]** 欄位並檢查 **[!UICONTROL Take the deliveries into account in the provisional calendar]** 選項：在計算時，會考慮在傳送日期前7天傳送的傳送以及在套用規則的傳送日期後7天排程的傳送。
...
>[!NOTE]
>
>當您變更型別規則的定義時，可以建立 **模擬** 為了控制其對套用它的傳送的影響，並監控傳送對彼此的影響。 有關詳細資訊，請參閱 [行銷活動模擬](campaign-simulations.md).
...
1. 在 **[!UICONTROL General]** 索引標籤中，選取 **[!UICONTROL Re-apply the rule at the start of personalization]** 選項。
   ...
   此選項會覆寫以下專案中所定義的值： **[!UICONTROL Frequency]** 欄位並在個人化階段自動套用規則。 有關詳細資訊，請參閱 [調整計算頻率](applying-rules.md#adjusting-calculation-frequency).
...
1. 選取 **[!UICONTROL Take the deliveries into account in the provisional calendar]** 包括已排程傳遞的選項。

   ![](assets/campaign_opt_pressure_example_1.png)

   計算時，會考慮在傳送日期前7天傳送的傳送以及在傳送日期後7天排程的傳送。 有關詳細資訊，請參閱 [設定期間](#setting-the-period).

   >[!AVAILABILITY]
   >Campaign v8無法考慮已排程的傳遞。
...
1. 將聯絡日期設定為隔天（目前日期+ 1天）。
   ...
   為了成功實作壓力規則排除，請確保將提取日期和時間設定在聯絡日期和時間之前，以及在重新套用夜間仲裁之前。 有關詳細資訊，請參閱 [仲裁後排除](#exclusion-after-arbitration).

1. 取消選取 **[!UICONTROL Confirm the delivery before sending]** 選項並儲存變更。
1. 對於您要傳送的每個傳遞，以類似方式執行。 請務必為每個傳遞設定所需的權重。
1. 執行相關工作流程以準備和傳送傳遞。

套用夜間仲裁時，將排除相同收件者具有較低權重的傳送。 只有具有最高權重的傳遞才會被視為傳送。 有關詳細資訊，請參閱 [訊息權重](#message-weight).
