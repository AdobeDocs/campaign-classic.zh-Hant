...
表單設定可讓您選取用來產生頁面的範本。 若要存取，請按一下 **[!UICONTROL Properties]** 按鈕，然後選取 **[!UICONTROL Rendering]** 標籤。 依預設，有許多範本（樣式表）可供使用。
...
* 變更用作標誌的影像，並調整其大小，
* 當使用者選取此演算範本時，也請指定存取預覽影像的路徑。

此 **[!UICONTROL Headers/Footers]** tab可讓您使用此範本變更顯示在每個表單頁面頁首和頁尾的資訊。
...
您可以透過相關標籤變更線段內容、新增框線及變更字型屬性。 按一下 **[!UICONTROL OK]** 以確認這些變更。
...
您可以為表單的每個元素（輸入欄位、影像、選項按鈕等）多載樣式表。
...
### 定義頁首與頁尾 {#defining-headers-and-footers}

欄位會以樹狀結構排序，其根目錄與頁面同名。 選取它以修改名稱。

視窗標題必須輸入於 **[!UICONTROL Page]** 表單屬性視窗的索引標籤。 您也可以新增一組內容至頁首與頁尾（此資訊將顯示在每個頁面上）。 此內容輸入在的相符區段中 **[!UICONTROL Texts]** 標籤，如下所示：

![](assets/s_ncs_admin_survey_titles_config.png)

### 將元素新增至HTML標題 {#adding-elements-to-html-header}

您可以輸入要插入表單頁面HTML標頭中的其他元素。 若要這麼做，請在 **[!UICONTROL Header]** 標籤內的任何專案。
...
當使用者填寫表單時，將會根據其格式或設定自動對特定欄位執行檢查。 這可讓您讓某些欄位成為必填欄位(請參閱 [定義必要欄位](#defining-mandatory-fields))或檢查所輸入資料的格式(請參閱 [檢查資料格式](#checking-data-format))。 檢查會在頁面核准期間執行（透過按一下可啟用輸出轉變的連結或按鈕）。

### 定義必要欄位 {#defining-mandatory-fields}
...
對於其值儲存在資料庫現有欄位中的表單檢查，將套用儲存欄位的規則。
...
您可以根據使用者選擇的值，設定頁面上要顯示的欄位顯示。 這可以套用至一個欄位或一組欄位（當它們分組在容器中時）。
...
* 此 **[!UICONTROL Recipients]** 子樹狀結構包含插入表單中並儲存在資料庫中的輸入欄位。

   有關詳細資訊，請參閱 [將資料儲存在資料庫中](web-forms-answers.md#storing-data-in-the-database).

* 此 **[!UICONTROL Variables]** 子樹狀結構包含此表單的可用變數。 有關詳細資訊，請參閱 [將資料儲存在區域變數中](web-forms-answers.md#storing-data-in-a-local-variable).
...
您可以從其他網路表單匯入欄位或容器。 這可讓您建立可重複使用的區塊資料庫，這些區塊將插入表單，例如地址區塊、電子報訂閱區域等。
