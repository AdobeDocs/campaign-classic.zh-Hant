...
>[!NOTE]
>
>進階運算式編輯會顯示在上 [此頁面](../../platform/using/defining-filter-conditions.md#list-of-functions).
...
您可以新增 **邊框** 至選取的區塊。 邊框是使用下列三個選項來定義：樣式、大小和顏色。
...
您可以將資料庫欄位與輸入區域、選項按鈕或核取方塊型別區塊連結。
...
此 **必要欄位** 選項可讓您僅在使用者已填入欄位時，授權核准頁面。 如果未填寫必填欄位，則會出現錯誤訊息。
...
* 將分項清單值（由所選欄位定義）輸入 **[!UICONTROL Value]** 欄位。

   ![](assets/dce_sidebar_completeoptionradio.png)

### 修改表單欄位 {#modifying-form-fields}

單選按鈕、輸入區域、下拉式清單等表單欄位。 可使用其工具列進行修改。

這表示您可以：

* 使用刪除包含表單欄位的區塊 **[!UICONTROL Delete]** 圖示。
* 透過使用建立新區塊來複製所選欄位 **[!UICONTROL Duplicate]** 圖示。
...
當使用者按一下按鈕時，您可以定義關聯的動作。 若要這麼做，請從下拉式清單中選取要執行的動作。
...
>[!NOTE]
>
>對於每種型別的連結，您可以設定開啟模式：在 **Target** 下拉式清單。 此值對應至 **`<target>`** HTML標籤。
>
>可用清單 **目標** 如下所示：
>
>* 其他(IFrame)
...
將相關連結地址輸入到 **URL** 欄位。 URL欄位應輸入為： **https://www.myURL.com**.

### 連結至Web應用程式 {#link-to-a-web-application}

此 **連結至Web應用程式** 選項可讓您存取Adobe Campaign Web應用程式。

![](assets/dce_toolbar_imgblock_appweb.png)

從對應的欄位中選取Web應用程式。

建議的Web應用程式清單對應於中可用的應用程式 **[!UICONTROL Resources > Online > Web Applications]** 節點。
...
* **加底線** 圖示：將選取的文字加上底線(文字換行 `<span style="text-decoration: underline;">` 標籤)
* **靠左對齊** icon：將文字對齊所選取區塊的左側(add style=&quot;text-align： left；&quot;)
* **置中** icon：將所選取區塊的文字置中對齊（新增style=&quot;text-align： center；&quot;）
* **靠右對齊** icon：將文字對齊所選取區塊的右側（新增style=&quot;text-align： right；&quot;）
* **變更背景顏色** icon：可讓您變更所選區塊的背景顏色(add style=&quot;background-color： rgba(170， 86， 255， 0.87))
* **變更文字色彩** 圖示：可讓您變更所選區塊的文字顏色，或僅變更所選文字(`<span style="color: #CODE">`)
...
若要插入影像，請選取「影像型別」區塊，然後按一下 **影像** 圖示。
...
此 **個人化欄位** 插入圖示的選項可讓您將資料庫欄位新增到內容中，例如收件者的名稱。 此選項僅適用於文字文字區塊。

![](assets/dce_toolbar_textblock_persofield.png)

依預設，提供的欄位來自 **[!UICONTROL Recipient]** 表格。 必要時，編輯Web應用程式屬性以選取其他表格。

欄位名稱會顯示在編輯器中，以黃色反白顯示。 產生個人化時（例如，預覽登入頁面時），收件者的設定檔會取代此欄位。

範例會顯示在 [插入個人化欄位](creating-a-landing-page.md#inserting-a-personalization-field) 區段。

### 插入個人化區塊 {#inserting-a-personalization-block}

此 **個人化區塊** 選項可讓您在內容中插入動態和個人化的區塊。 例如，您可以新增標誌或問候語。 它不適用於文字型別區塊。

![](assets/dce_toolbar_textblock_persoblock.png)

插入後，個人化區塊名稱會顯示在編輯器中，以黃色反白顯示。 產生個人化時，會自動調整成適合收件者設定檔。

如需內建個人化區塊的詳細資訊，以及如何定義自訂個人化區塊，請參閱 [此頁面](../../delivery/using/personalization-blocks.md).
