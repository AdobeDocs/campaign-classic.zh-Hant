...
+ 開始使用 {#getting-started}
   ...
   + {hide-from-toc}[字彙表 — 隱藏](platform/using/ac-glossary.md)
   + 隱私權 {#privacy}
   ...
   + 輪廓管理 {#profile-management}
      ...
      + {hide-from-toc}[編輯設定檔 — 隱藏](platform/using/editing-a-profile.md)
      + {hide-from-toc}[新增設定檔 — 隱藏](platform/using/adding-profiles.md)
      + {hide-from-toc}[管理設定檔 — 隱藏](platform/using/managing-profiles.md)
      + [更新資料](platform/using/updating-data.md)
      + {hide-from-toc}[匯入和匯出設定檔 — 隱藏](platform/using/exporting-and-importing-profiles.md)
...
+ 建立及傳送訊息 {#sending-messages}
   ...
   + 快速入門 {#key-steps-when-creating-a-delivery}
      + {hide-from-toc}[開始傳遞 — 隱藏](delivery/using/steps-about-delivery-creation-steps.md)
      + {hide-from-toc}[建立並識別傳遞 — 隱藏](delivery/using/steps-create-and-identify-the-delivery.md)
      + {hide-from-toc}[定義對象 — 隱藏](delivery/using/steps-defining-the-target-population.md)
      ...
      + {hide-from-toc}[傳遞最佳實務 — 隱藏](delivery/using/delivery-best-practices.md)
   + 關於傳遞的最佳實務 {#delivery-bestpractices}
      ...
      + {hide-from-toc}[建置個人化內容 — 隱藏](delivery/using/design-and-personalize.md)
      + {hide-from-toc}[定義正確的對象 — 隱藏](delivery/using/define-the-right-audience.md)
   ...
   + 傳送簡訊 {#sending-messages-on-mobiles}
      ...
      + {hide-from-toc}[移轉至擴充通用SMPP聯結器](delivery/using/unsupported-connector-migration.md)
   ...
   + 使用傳遞範本 {#using-delivery-templates}
      ...
      + {hide-from-toc}[從範本建立傳遞 — 隱藏](delivery/using/creating-a-delivery-from-a-template.md)
   + 個人化傳遞 {#personalizing-deliveries}
      ...
      + {hide-from-toc}[個人化優惠券 — 隱藏](delivery/using/personalized-coupons.md)
      + {hide-from-toc}[產生個人化PDF檔案 — 隱藏](delivery/using/generating-personalized-pdf-documents.md)
...
+ 協調行銷活動 {#orchestrating-campaigns}
   ...
   + 協調行銷活動 {#orchestrate-campaigns}
      ...
      + {hide-from-toc}[選取對象 — 隱藏](campaign/using/marketing-campaign-target.md)
      + {hide-from-toc}[管理檔案和資產 — 隱藏](campaign/using/marketing-campaign-assets.md)
      + {hide-from-toc}[設定和管理核准 — 隱藏](campaign/using/marketing-campaign-approval.md)
   ...
   + 行銷活動最佳化 {#campaign-optimization}
      + {hide-from-toc}[關於行銷活動型別 — 隱藏](campaign-opt/using/about-campaign-typologies.md)
      + {hide-from-toc}[壓力規則 — 隱藏](campaign-opt/using/pressure-rules.md)
      + {hide-from-toc}[一致性規則 — 隱藏](campaign-opt/using/consistency-rules.md)
      + {hide-from-toc}[控制項規則 — 隱藏](campaign-opt/using/control-rules.md)
...
+ 行銷資源管理 {#mrm}
   ...
   + {hide-from-toc}[建立和管理工作 — 隱藏](mrm/using/creating-and-managing-tasks.md)
   + {hide-from-toc}[控制成本 — 隱藏](mrm/using/controlling-costs.md)
   + {hide-from-toc}[管理行銷資源 — 隱藏](mrm/using/managing-marketing-resources.md)
   + {hide-from-toc}[討論區 — 隱藏](mrm/using/discussion-forums.md)
+ 分散式行銷 {#distributed-marketing}
   + {hide-from-toc}[關於分散式行銷 — 隱藏](distributed/using/about-distributed-marketing.md)
   + {hide-from-toc}[建立本機行銷活動 — 隱藏](distributed/using/creating-a-local-campaign.md)
   + {hide-from-toc}[建立合作行銷活動 — 隱藏](distributed/using/creating-a-collaborative-campaign.md)
...
+ 整合 ADobe Experience Cloud {#integrating-with-adobe-experience-cloud}
   ...
   + ACS Connector {#acs-connector}
      ...
      + {hide-from-toc}[同步設定檔 — 隱藏](integrations/using/synchronizing-profiles.md)
      + {hide-from-toc}[同步受眾 — 隱藏](integrations/using/synchronizing-audiences.md)
      + {hide-from-toc}[同步處理網頁應用程式 — 隱藏](integrations/using/synchronizing-web-applications.md)
      + {hide-from-toc}[疑難排解 [!DNL ACS Connector]  — 隱藏](integrations/using/troubleshooting-the-acs-connector.md)
...
+ 使用工作流程自動化 {#automating-with-workflows}
   + 開始使用工作流程 {#introduction}
      ...
      + {hide-from-toc}[建置工作流程 — 隱藏](workflow/using/building-a-workflow.md)
      ...
      + {hide-from-toc}[工作流程最佳實務 — 隱藏](workflow/using/workflow-best-practices.md)
   ...
   + 目標定位活動 {#targeting-activities}
      ...
      + {hide-from-toc}[聯集 — 隱藏](workflow/using/union.md)
      ...
      + {hide-from-toc}[依儲存格列出的優惠 — 隱藏](workflow/using/offers-by-cell.md)
      ...
      + {hide-from-toc}[優惠方案引擎 — 隱藏](workflow/using/offer-engine.md)
   ...
   + 動作活動 {#action-activities}
      ...
      + {hide-from-toc}[週期性傳遞 — 隱藏](workflow/using/recurring-delivery.md)
      + {hide-from-toc}[跨頻道傳遞 — 隱藏](workflow/using/cross-channel-deliveries.md)
      + {hide-from-toc}[本機核准 — 隱藏](workflow/using/local-approval.md)
      + {hide-from-toc}[資料載入(RDBMS) — 隱藏](workflow/using/data-loading-rdbms.md)
      + {hide-from-toc}[載入中(SOAP) — 隱藏](workflow/using/loading-soap.md)
      + {hide-from-toc}[資料載入（檔案） — 隱藏](workflow/using/data-loading-file.md)
      + {hide-from-toc}[內容管理 — 隱藏](workflow/using/content-management.md)
      + {hide-from-toc}[資料擷取（檔案） — 隱藏](workflow/using/extraction-file.md)
      + {hide-from-toc}[SQL程式碼和JavaScript程式碼 — 隱藏](workflow/using/sql-code-and-javascript-code.md)
      + {hide-from-toc}[SQL資料管理 — 隱藏](workflow/using/sql-data-management.md)
      + {hide-from-toc}[Nlserver模組 — 隱藏](workflow/using/nlserver-module.md)
   ...
   + 使用案例 {#use-cases}
      ...
      + 傳遞 {#deliveries}
         + {hide-from-toc}[使用本機核准活動 — 隱藏](workflow/using/using-the-local-approval-activity.md)
         ...
         + {hide-from-toc}[跨通道傳遞工作流程 — 隱藏](workflow/using/cross-channel-delivery-workflow.md)
         + {hide-from-toc}[包含自訂日期欄位的電子郵件擴充 — 隱藏](workflow/using/email-enrichment-with-custom-date-fields.md)
   + 監視 {#monitoring}
      + {hide-from-toc}[傳送報告至清單 — 隱藏](workflow/using/sending-a-report-to-a-list.md)
      + {hide-from-toc}[監督您的工作流程 — 隱藏](workflow/using/supervising-workflows.md)
      + {hide-from-toc}[傳送個人化警示給營運商 — 隱藏](workflow/using/sending-personalized-alerts-to-operators.md)
   ...
   + 查詢和篩選 {#query-and-filter-hidden}
      ...
      + {hide-from-toc}[執行彙總運算 — 隱藏](workflow/using/performing-aggregate-computing.md)
      + {hide-from-toc}[使用分組管理進行查詢 — 隱藏](workflow/using/querying-using-grouping-management.md)
      + {hide-from-toc}[使用多對多關係進行查詢 — HIDDEN](workflow/using/querying-using-many-to-many-relationship.md)
      + {hide-from-toc}[新增分項清單型別計算欄位 — 隱藏](workflow/using/adding-enumeration-type-calculated-field.md)
   ...
   + 監視工作流程 {#monitoring-workflows}
      + {hide-from-toc}[監視工作流程執行 — 隱藏](workflow/using/monitoring-workflow-execution.md)
   ...
   + 進階管理 {#advanced-management}
      ...
      + {hide-from-toc}[進階引數 — 隱藏](workflow/using/advanced-parameters.md)
      + {hide-from-toc}[JavaScript指令碼和範本 — 隱藏](workflow/using/javascript-scripts-and-templates.md)
      + {hide-from-toc}[工作流程中的JavaScript程式碼範例 — 隱藏](workflow/using/javascript-in-workflows.md)
      + {hide-from-toc}[存取外部資料庫 — 隱藏](workflow/using/accessing-an-external-database-fda.md)
      + {hide-from-toc}[管理許可權 — 隱藏](workflow/using/managing-rights.md)
...
+ 安裝及設定指南 {#installing-campaign-classic}
   ...
   + 設定 Campaign {#additional-configurations}
      ...
      + {hide-from-toc}[搭配行銷活動使用MX伺服器](installation/using/using-mx-servers.md)
   ...
   + 附錄 {#appendices}
      ...
      + {hide-from-toc}[解除安裝行銷活動](installation/using/uninstalling-campaign.md)
...
+ 監視指南 {#monitoring-campaign-classic}
   ...
   + 更新 Adobe Campaign {#updating-adobe-campaign}
      ...
      + {hide-from-toc}[移轉程式](production/using/migration-procedures.md)
...
+ 移轉指南 {#migrating-to-a-new-version}
   + {hide-from-toc}[移轉至Adobe Campaign v7](migration/using/about-migration.md)
   ...
   + 開始移轉 {#migrating-to-adobe-campaign-7}
      + {hide-from-toc}[移轉Microsoft Windows平台](migration/using/migrating-in-windows-for-adobe-campaign-7.md)
      + {hide-from-toc}[移轉Linux平台](migration/using/migrating-in-linux-for-adobe-campaign-v7.md)
   + {hide-from-toc}[復原程式](migration/using/about-rollback.md)
+ 技術說明 {#technotes}
   ...
   + 升級與相容性{#technote-migration}
      + {hide-from-toc}[技術備註](technotes/using/technote.md)
      + [啟用 Microsoft Edge Chromium](technotes/using/edge-chromium.md)
      + {hide-from-toc}[新的傳遞能力伺服器](technotes/using/deliverability-server.md)
      + {hide-from-toc}[Apache安全性升級](technotes/using/acc-apache-upgrade.md)
      + {hide-from-toc}[2023系統升級](technotes/using/tech-stack-upgrade.md)
      + {hide-from-toc}[IMS更新](technotes/using/ims-updates.md)
      + {hide-from-toc}[終止TLS 1.0和1.1支援](technotes/using/eol-tls-support.md)
   + 電子郵件和推播通知{#technotes-email}
      ...
      + {hide-from-toc}[在ISP中斷後更新不正確的硬退信](delivery/using/update-bounces.md)
      + [iOS 憑證更新](technotes/using/ios-certificate-update.md)
      + {hide-from-toc}[追蹤的URL更新](technotes/using/tracked-urls.md)
