...
通知訊息會傳送給[!DNL Adobe Campaign]操作員，這些操作員是指定的稽核者，以通知他們核准請求。 檢查檢閱者是否具有核准的&#x200B;**適當許可權**，以及其安全區域是否已正確定義。 [進一步瞭解如何選取稽核者](#selecting-reviewers)。

在[核准程式概述](#checking-and-approving-deliveries)中呈現核准程式。

>[!NOTE]
>
>只有傳遞擁有者可以開始傳遞。 為了讓其他操作員（或操作員群組）能夠開始傳遞，您必須在&#x200B;**[!UICONTROL Delivery start:]**&#x200B;欄位中將他們新增為稽核者。\
>[進一步瞭解如何選取稽核者](#selecting-reviewers)。

## 操作原則 {#operating-principle-}

例如，預算核准的標準訊息如下：

![含驗證連結的核准通知電子郵件](assets/s_user_validation_link_in_mail.png)

檢閱者操作員可以選擇是否核准預算。

![核準確認頁面，包含接受或拒絕選項](assets/s_user_validation_page_confirm.png)

操作員驗證後，工作的核准或拒絕將轉送到傳遞儀表板。

![顯示工作核准連結的行銷活動儀表板](assets/s_user_validation_link_in_op_board.png)

此資訊也可在行銷活動的核准記錄中取得。 這些記錄檔是透過&#x200B;**[!UICONTROL Edit > Tracking > Approvals]**&#x200B;索引標籤來存取。

![行銷活動編輯索引標籤顯示核准記錄](assets/s_user_validation_log_in_op_edit_tab.png)
...
![主控台中的行銷活動儀表板核准動作](assets/s_user_validation_from_console.png)
...
![通知連結的核准登入頁面](assets/s_user_validation__log_in.png)

選取&#x200B;**[!UICONTROL Accept]**&#x200B;或&#x200B;**[!UICONTROL Reject]**，並視需要輸入註解。

![核准頁面，包含接受或拒絕及註解](assets/s_user_validation_save_target_validation.png)
...
* 也可以透過每個傳遞的&#x200B;**[!UICONTROL Tracking > Approvals]**&#x200B;索引標籤存取此資訊：
  ...
  ![傳遞追蹤核准索引標籤](assets/s_user_validation_log_from_exe_tab.png)
...
建立目標定位工作流程時，如果核准是自動的（預設模式），[!DNL Adobe Campaign]會顯示核准連結，或在需要核準時立即傳送通知。
...
![使用手動和自動模式的核准設定](assets/s_user_validation_select_mode.png)
...
![已停用通知的核准設定](assets/s_user_validation_notif_desactivate.png)
...
[!DNL Adobe Campaign]可讓您以合作模式，針對行銷活動的主要階段設定核准流程。

如果是直接郵件傳遞，[!DNL Adobe Campaign]操作員可以在解壓縮檔案傳送給路由器之前檢視解壓縮檔案，必要時可以變更格式並重新啟動解壓縮。 請參閱[核准擷取檔案](#approving-an-extraction-file)。

對於每個行銷活動，您可以核准傳遞目標、內容（請參閱[核准內容](#approving-content)）和成本。 可以透過電子郵件通知負責核准的[!DNL Adobe Campaign]操作者，他們可透過主控台或網路連線核准或拒絕核准。 請參閱[核准傳遞的步驟](#approving-processes)。

完成這些驗證階段後，即可啟動傳遞。 [進一步瞭解如何開始傳遞](../../campaign/using/marketing-campaign-deliveries.md#starting-a-delivery)。
...
![顯示傳遞狀態的行銷活動儀表板](assets/s_ncs_user_edit_del_to_validate.png)

>[!NOTE]
>
>在目標定位工作流程中，如果訊息準備期間發生連結至設定問題的錯誤，控制面板上會顯示&#x200B;**[!UICONTROL Restart message preparation]**&#x200B;連結。 更正錯誤，然後按一下此連結，在略過目標階段時重新啟動訊息準備。

![重新啟動訊息準備的儀表板連結](assets/s_user_validation_relaunch_message_preparation.png)
...
* **摘取核准（直接郵件傳遞）**
  ...
  ![顯示核准內容連結的核准儀表板](assets/s_ncs_user_edit_file_valid.png)

  擷取檔案可透過核准方塊預覽，然後接受或拒絕。

  ![在核准對話方塊中預覽擷取檔案](assets/s_ncs_user_edit_file_valid_preview_file.png)
...
* **核准關聯的傳遞**
  ...
  **[!UICONTROL Enable individual approval of each associated delivery]**&#x200B;選項用於與次要傳遞相關的主要傳遞。 依預設，不會選取此選項，因此可以執行主要傳送的整體核准。 如果選取此選項，則必須個別核准每個傳遞。
  ...
  ![啟用關聯傳遞的個別核准選項](assets/s_ncs_user_task_valid_associate.png)

### 選取核准的程式 {#choosing-the-processes-to-be-approved}

核准階段是使用與行銷活動相關聯的範本來定義。 您必須從範本中選取要核准的元素，並指定負責這些核准的[!DNL Adobe Campaign]運運算元。 如需行銷活動範本的詳細資訊，請參閱[行銷活動範本](../../campaign/using/marketing-campaign-templates.md#campaign-templates)。
...
您可以選取和取消選取要核准的程式，並指定負責核准的[!DNL Adobe Campaign]操作員。 這些可以是個別運運算元、一組運運算元或運運算元清單。
...
![核准行事曆和提醒設定](assets/s_ncs_user_edit_op_valid_calendar.png)

對於每個傳遞，按一下「**[!UICONTROL Audit]**」按鈕和「**[!UICONTROL Approvals]**」標籤以檢視和編輯核准日期及自動提醒。

![包含日期和提醒的傳遞核准索引標籤](assets/s_ncs_user_edit_del_valid.png)
...
>[!CAUTION]
>
>若要核准內容，校訂週期是強制性的。 校樣可讓您核准資訊顯示、個人化資料並檢查連結是否正常運作。 瞭解如何在[建立校訂](../../delivery/using/steps-validating-the-delivery.md#sending-a-proof)中建立校訂。
>
>下文詳述的內容核准功能與證明傳送有關。
...
1. 建立新傳遞後，行銷活動經理按一下行銷活動控制面板上的&#x200B;**[!UICONTROL Submit content]**&#x200B;連結，以開始內容核准週期。
   ...
   ![提交內容以供核准的Campaign儀表板連結](assets/s_ncs_user_validation_submit_content_validation.png)
...
1. 系統會傳送通知電子郵件給負責內容的人員，由其選擇是否核准：
   ...
   * 透過通知電子郵件：
     ...
     ![校訂的內容核准通知電子郵件](assets/s_ncs_user_del_content_valid_bat_notif.png)
   ...
   * 透過主控台或網頁介面、傳遞追蹤、傳遞控制面板或行銷活動控制面板：
     ...
     ![顯示內容校訂清單的傳遞追蹤](assets/s_ncs_user_validation_content_bat_op.png)

     >[!NOTE]
     >
     >此行銷活動儀表板可讓您按一下&#x200B;**[!UICONTROL Inbox rendering...]**&#x200B;連結，以檢視已傳送的校樣清單。 若要檢視其內容，請按一下清單右側的&#x200B;**[!UICONTROL Detail]**&#x200B;圖示。

     ![內容核准的校訂詳細資料檢視](assets/s_ncs_user_validation_content_BAT_details.png)
...
1. 系統會傳送通知電子郵件給行銷活動的負責人，通知其內容是否已核准。
   ...
   ![重新啟動內容核准的Campaign儀表板連結](assets/s_user_validation_relaunch_content_validation.png)
...
1. 建立新傳遞後，行銷活動負責人按一下行銷活動控制面板中的&#x200B;**[!UICONTROL Submit content editing]**&#x200B;連結，以開始內容編輯週期。
   ...
   ![提交內容編輯的Campaign儀表板連結](assets/s_ncs_user_validation_submit_content_edition.png)

1. 負責內容編輯的人員會收到電子郵件，通知他們內容可用。

   ![內容編輯通知電子郵件](assets/s_ncs_user_validation_submit_content_notif.png)

1. 接著，他們可以登入主控台、開啟傳遞內容，並使用簡化的助理員加以編輯，以變更主題、HTML和文字內容，以及傳送校樣。

   ![編輯傳遞內容的簡化助理](assets/s_user_validation_content_edition.png)
...
1. 當負責內容編輯的人員完成傳送內容的任何變更後，他們就可以提供內容。
   ...
   * 按一下通知訊息中的連結，然後核准內容可用性。
     ...
     ![用於核准內容可用性的通知連結](assets/s_ncs_user_validation_submit_content_available2.png)

     操作員可在將內容提交至行銷活動負責人之前新增評論。

     提交內容可用性前![註解欄位](assets/s_ncs_user_validation_submit_content_available3.png)

     通知訊息可讓稽核者核准或拒絕內容。

     ![內容可用性的核准回應](assets/s_ncs_user_validation_submit_content_available4.png)
...
1. 通知電子郵件包含已傳送校樣的連結（可讓您檢視傳遞呈現），以及核准或拒絕傳遞內容的按鈕。
   ...
   ![包含校訂連結的外部內容核准電子郵件](assets/s_user_validation_external_content.png)

### 核准擷取檔案 {#approving-an-extraction-file}

對於離線傳遞，[!DNL Adobe Campaign]會產生擷取檔案，視其設定方式而定，會傳送至路由器。 其內容取決於所使用的匯出範本。

核准內容、目標定位和預算後，傳遞會變更為&#x200B;**[!UICONTROL Extraction pending]**，直到啟動行銷活動的擷取工作流程為止。

![傳遞狀態顯示正在等候擷取](assets/s_ncs_user_waiting_file_extraction.png)

在擷取請求日期建立擷取檔案，且傳遞狀態變更為&#x200B;**[!UICONTROL File to approve]**。

![傳遞狀態顯示要核准的檔案](assets/s_ncs_user_file_extract_to_valid.png)
