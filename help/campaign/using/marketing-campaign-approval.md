...
通知訊息會傳送給Adobe Campaign操作者，這些操作者會成為指定的稽核者，以通知他們核准請求。 檢查檢閱者是否擁有 **適當的許可權** 以核准，且其安全區域已正確定義。 [了解更多](#selecting-reviewers)。

核准程式會顯示在中 [本節](#checking-and-approving-deliveries).

>[!NOTE]
>
>只有傳遞擁有者可以開始傳遞。 為了讓其他操作員（或操作員群組）能夠開始傳遞，您必須將其新增為中的稽核者 **[!UICONTROL Delivery start:]** 欄位。\
...
所有需要核准的工作都會在行銷活動範本中選取( **[!UICONTROL Properties]** > **[!UICONTROL Advanced campaign settings...]** > **[!UICONTROL Approvals]** 標籤)，負責核准的操作者亦然（除非未啟用此選項，否則他們會收到通知）。 如需詳細資訊，請參閱[本章節](#approving-processes)。

您可以為使用此範本建立的每個行銷活動覆寫這些設定，也可以為每個行銷活動傳遞個別覆寫：按一下 **[!UICONTROL Properties]** 按鈕，然後 **[!UICONTROL Approvals]** 標籤。
...
對於每種核准型別，會從傳送的下拉式清單中選取負責核准的運運算元或運運算元群組。 可使用新增更多運運算元 **[!UICONTROL Edit...]** 連結。 此視窗也可讓您編輯核准期限。

![](assets/s_user_validation_add_operator.png)

若未指定稽核者，則行銷活動經理將負責核准並接收通知。 行銷活動管理員指定於 **[!UICONTROL Edit > Properties]** 行銷活動的索引標籤：

![](assets/s_user_op_manager_field.png)

>[!NOTE]
>
>所有其他的Adobe Campaign運運算元，搭配 **[!UICONTROL Administrator]** 許可權也可以核准工作，但他們不會收到通知。\
>依預設，如果已定義核准操作員，行銷活動經理無法執行核准或開始傳送。 您可以修改此行為，並透過建立 **NmsCampaign_Activate_OwnerConfirmation** 選項和 **1** 作為值。
...
勾選要核准的資訊，選擇是否接受或拒絕核准，並視需要輸入註解。 按一下 **[!UICONTROL Ok]** 以儲存。
...
* 在行銷活動核准記錄中， **[!UICONTROL Approvals]** 的子標籤 **[!UICONTROL Edit > Tracking]** 標籤：

  ![](assets/s_user_validation_log_from_op.png)

* 在行銷活動傳遞記錄中， **[!UICONTROL Deliveries]** 的子標籤 **[!UICONTROL Edit > Tracking]** 標籤：

  ![](assets/s_user_validation_log_from_delivery_list.png)

* 若要檢視每個傳遞的核准狀態，請按一下 **[!UICONTROL Hide/show log]** 的選項 **[!UICONTROL Summary]** 標籤。
...
若要選擇核准模式（手動或自動），請按一下 **[!UICONTROL Edit > Properties]** 行銷活動或行銷活動範本的標籤，然後按一下 **[!UICONTROL Advanced campaign settings...]** 最後 **[!UICONTROL Approvals]** 標籤。
...
>[!NOTE]
>
>在目標定位工作流程中，如果訊息準備期間發生連結至設定問題的錯誤，則 **[!UICONTROL Restart message preparation]** 連結會顯示在控制面板上。 更正錯誤，然後按一下此連結，在略過目標階段時重新啟動訊息準備。
...
* **目標定位、內容和預算**
  ...
  當 **[!UICONTROL Enable target approval]**， **[!UICONTROL Enable content approval]** 或 **[!UICONTROL Enable budget approval]** 在「工作核准設定」視窗中選取選項，相關連結會顯示在相關傳送的campaign控制面板中。
...
* **摘取核准（直接郵件傳送）**
  ...
  一個 **[!UICONTROL Approve content]** 行銷活動控制面板上提供連結，如下所示：
...
* **核准關聯的傳遞**
  ...
  此 **[!UICONTROL Enable individual approval of each associated delivery]** 選項適用於與次要傳遞相關聯的主要傳遞。 依預設，不會選取此選項，因此可以執行主要傳送的整體核准。 如果選取此選項，則必須個別核准每個傳遞。

  ![](assets/s_ncs_user_task_valid_associate.png)

### 選取核准的程式 {#choosing-the-processes-to-be-approved}

核准階段是使用與行銷活動相關聯的範本來定義。 您必須從範本中選取要核准的元素，並指定負責這些核准的Adobe Campaign運運算元。 有關行銷活動範本的詳細資訊，請參閱 [本節](../../campaign/using/marketing-campaign-templates.md#campaign-templates).
...
若為促銷活動，請按一下 **[!UICONTROL Edit > Properties]** 標籤，然後 **[!UICONTROL Advanced campaign settings...]** 連結，最後連結 **[!UICONTROL Approvals]** 子標籤以存取核准設定頁面。

您可以選取和取消選取要核准的程式，並指定負責核准的Adobe Campaign操作者。 這些可以是個別運運算元、一組運運算元或運運算元清單。

若要選取運運算元清單，請按一下 **[!UICONTROL Edit...]** 指向欄位右側的連結，指定第一個檢閱者，並視需要新增運運算元，如下所示：
...
對於每個傳遞，按一下 **[!UICONTROL Audit]** 按鈕和 **[!UICONTROL Approvals]** 標籤以檢視和編輯核准日期和自動提醒。
...
>[!CAUTION]
>
>若要核准內容，校訂週期是強制性的。 校樣可讓您核准資訊顯示、個人化資料並檢查連結是否正常運作。 瞭解如何在中建立校訂 [本節](../../delivery/using/steps-validating-the-delivery.md#sending-a-proof).
>
>下文詳述的內容核准功能與證明傳送有關。
...
1. 系統會傳送通知電子郵件給負責內容的人員，由其選擇是否核准：
   ...
   * 透過通知電子郵件：
     ...
     >[!NOTE]
     >
     >通知電子郵件包含已傳送校樣的連結，如果符合下列條件，則可能包含各種網頁郵件之訊息的轉譯連結： **傳遞能力** 選項已針對此執行個體啟用。
   ...
   * 透過主控台或網頁介面、傳遞追蹤、傳遞控制面板或行銷活動控制面板：
     ...
     >[!NOTE]
     >
     >此行銷活動儀表板可讓您按一下 **[!UICONTROL Inbox rendering...]** 連結。 若要檢視其內容，請按一下 **[!UICONTROL Detail]** 圖示加以儲存。
