...
1. 按一 **[!UICONTROL New]** 下或在範本清單中按一下滑鼠右鍵，然後 **[!UICONTROL New]** 在下拉式清單中選擇。
1. 輸入假設標籤。
1. 指定範本是針對選件或透過的傳送進行假設 **[!UICONTROL Hypothesis type]**。
1. 對於 **[!UICONTROL Delivery]** 類型模板，請指定測量應使用或不使用控制組(有關詳細資訊，請參閱 [假設模板的屬性](#properties-of-a-hypothesis-template))。
1. 對於 **[!UICONTROL Delivery]** 類型範本，您可以選擇特定渠道，或決定使用下拉式清單將範本套用至Adobe Campaign中的所有可用渠道(如需詳細資訊，請參閱 **[!UICONTROL Channel]** 假設範本的屬性 [](#properties-of-a-hypothesis-template))。
1. 選擇 **[!UICONTROL Execution folder]** 要在其中建立的假設，並自動執行將從此模板建立的假設。
1. 選擇執行設定(如需詳細資訊，請參閱假設范 [本執行設定](#hypothesis-template-execution-settings))。
1. 指定假設計算期間(有關詳情，請參閱假設 [模板執行設定](#hypothesis-template-execution-settings))。

   >[!CAUTION]
   >
   >此期間由聯絡日期確定。

1. 在標籤 **[!UICONTROL Transactions]** 中，指定假設計算所需的表格和欄位(如需詳細資訊，請參閱「事 [務](#transactions)」)。
1. 如果您的範本已針對類型假 **[!UICONTROL Offer]** 設設定，您可以啟用 **[!UICONTROL Update offer proposition status]** 選項：在此情況下，選取您要變更的選件提案狀態。
1. 指定假設應用程式的範圍(如需詳細資訊，請參閱 [假設周長](#hypothesis-perimeter))。
1. 如有必要，請使用指令碼完成篩選(如需詳細資訊，請參閱 [假設周界](#hypothesis-perimeter))。

### 假設模板的性質 {#properties-of-a-hypothesis-template}

範本的標籤可 **[!UICONTROL General]** 讓您指定一般範本選項。 可用欄位包括：
...
* **[!UICONTROL Hypothesis type]**:可讓您判斷範本是否應以傳送或選件的假設為目標。
   ...
   >[!NOTE]
   >
   >如果範本套用至選件，則選 **[!UICONTROL Update offer proposition status]** 項卡中會提供該選 **[!UICONTROL Transactions]** 項。
...
* **[!UICONTROL Measurement with control group]**:可讓您指出是否已為傳送或促銷活動定義控制群組，並將其納入測量指標。 控制群組（未接收傳送）可讓您比較傳送後促銷活動與已接收傳送的目標人口，以評估促銷活動的影響。
   ...
   有關定義和配置控制組的詳細資訊，請參閱定 [義控制組](../../campaign/using/marketing-campaign-deliveries.md#defining-a-control-group)。

* **[!UICONTROL Channel]**:您可以選擇特定渠道，或從下拉式清單中選取，讓假設範本可用於Adobe Campaign **[!UICONTROL All channels]** 控制台中的所有渠道。 如果您設定特定渠道的範本，這可讓您在建立假設時，自動篩選每個渠道的傳送(請參 [閱建立假設](../../campaign/using/creating-hypotheses.md))。
...
範本的標籤 **[!UICONTROL General]** 也可讓您指定假設執行參數。 可用的選項如下：
...
* **[!UICONTROL Automatic execution]**:如有必要，可讓您排程假設重新計算（例如，如果您想定期更新指標，直到傳送結束）。
   ...
   1. 按一下 **[!UICONTROL Frequency of execution...]** 連結，然後按 **[!UICONTROL Change...]** 鈕。
...
此標籤包含各種欄位和表格，可讓您儲存交易中收件者反應的歷史記錄。 有關響應管 [理專用表](../../configuration/using/about-schema-reference.md) ，請參閱配置指南。

* **[!UICONTROL Schema (reaction log storage)]**:選擇收件人反應表。 Adobe Campaign中的現成可用表是 **NmsRemaMatchRcp**。
...
* **[!UICONTROL Measurement period]**:可讓您設定開始和結束日期，在此期間執行假設並復原購買行。
   ...
   如果假設是在飛行中啟動的，如果想立即觸發，就可以強制推動。 否則，系統會根據設定的計算結束日期自動觸發此假設(請參閱「在傳送時即時建立假設」 [](../../campaign/using/creating-hypotheses.md#creating-a-hypothesis-on-the-fly-on-a-delivery))。
...
* **指令碼**:您可以使用JavaScript指令碼，在假設設定執行期間動態超載假設設定。
   ...
   若要這麼做，請按一下連 **[!UICONTROL Advanced settings]** 結，然後輸入所要的指令碼。
...
1. 選擇頻 **[!UICONTROL Direct mail]** 道。

   >[!NOTE]
   >
   >由於模板專用於直接郵件遞送，因此使用此模型建立的假設可能不連結到任何其他遞送類型。

1. 在標籤 **[!UICONTROL Transactions]** 中，選取收件者反應表格。

   ![](assets/response_hypothesis_model_example_006.png)

1. 在欄位 **[!UICONTROL Transactions schema]** 中，選擇您的購買表格。

   ![](assets/response_hypothesis_model_example_007.png)

1. 在欄位中選擇購買 **[!UICONTROL Querying schema]** 行。
...
1. 在標籤 **[!UICONTROL Scope]** 中，按一下 **[!UICONTROL Edit query]** 以建立假設的篩選。

   ![](assets/response_hypothesis_model_example_011.png)

   如此建立的範本可讓您對購買表格中的產品或文章執行假設。

1. 按一 **[!UICONTROL Save]** 下以記錄範本。
