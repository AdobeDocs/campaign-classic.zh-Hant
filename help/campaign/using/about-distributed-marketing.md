...
# 關於分散式行銷{#about-distributed-marketing}
...
>[!CAUTION]
>
>「分散式行銷」模組是「促銷 **活動** 」選項。 請檢查您的授權合約。

## 術語 {#terminology}

### 中央實體 {#central-entities}
...
### 促銷活動套件清單 {#list-of-campaign-packages}

促銷活動套件清單包含可供本機實體使用的促銷活動。

### 促銷活動套件 {#campaign-package}

範本（或促銷活動例項）由中央實體建立，並可供一組本機實體使用。

### 本機促銷活動 {#local-campaign}

本機促銷活動是從具有特定執行排程的清單中參考的範本 **[!UICONTROL campaign packages]** 所建立 **的例項**。 其目標是使用由中央實體設定和設定的促銷活動範本，來滿足本端通訊需求。

地方實體的自治程度取決於所使用的實施。

請參閱建 [立本機促銷活動](../../campaign/using/creating-a-local-campaign.md)。

### 協作宣傳 {#collaborative-campaign}

協作促銷活動是由本機實 **體可使用的中央實體** ，定義執行排程的促銷活動。 每個本機實體的內容都維持不變，但會分擔成本。 若要參與，本機實體會訂閱協作促銷活動。
...
* **[!UICONTROL Collaborative campaign (by campaign)]**:針對數十個地方實體的促銷活動建議。 此類型的促銷活動會為每個本機實體建立子促銷活動。 在中央 **[!UICONTROL collaborative campaign (by campaign)]** 實體核准後，促銷活動就可供本機實體使用，由本機實體加以修改。 執行會在父促銷活動和子促銷活動之間自動同步。 本機實體必須擁有例項的存取權，才能訂購促銷活動並參與其中。
* **[!UICONTROL Collaborative campaign (by target approval)]**:針對數千個本地實體的促銷活動建議。 本地實體接收由中央實體預先定義的聯繫人清單。 本機實體會根據促銷活動內容，透過Web表單決定是否保留某些連絡人。 從選定接觸清單中推導出局部圖元。 若要參與促銷活動，本機實體只需要Web存取權。
* **[!UICONTROL Collaborative campaign (simple)]**:此模式可確保與舊版的特定執行程式相容。

請參閱建 [立協作促銷活動](../../campaign/using/creating-a-collaborative-campaign.md)。

### 訂購促銷活動套件 {#ordering-campaign-packages}

如果本機實體註冊促銷活動，則會將此訂單轉換為重新群組所有與促銷活動本地化相關資訊的順序。

## 工作區 {#workspace}

可從「促銷活動」範圍存取促銷活動套 **件清單** :按一下連 **[!UICONTROL Campaign packages]** 結。
...
## 運算子與實體 {#operators-and-entities}

首先，透過資料夾指定中央和本機實體運算 **[!UICONTROL Access management]** 子。
...
要建立組織實體，請按一下該節 **[!UICONTROL Administration > Access management > Organizational entities]** 點，然後按一下實 **[!UICONTROL New]** 體清單上方的表徵圖。
...
>[!NOTE]
>
>審批流程會顯示在「審批 [流程」部分](../../campaign/using/creating-a-local-campaign.md#approval-process) 。
...
1. 指定促銷活動本地化所需的欄位集：
   ...
   * 目標定義和最大尺寸，
   * 內容定義，
   * 執行排程（連絡日期和擷取日期），僅 **適用於本機運算子**,
   * 擴展的順序模式，並包含所有必要的附加欄位。

1. 建立網頁表單（Adobe或外部網站），讓您顯示本地化參數、評估目標和預算，以及預覽內容並核准訂單。

   對於 **協作促銷活動（透過目標核准）**，請建立表格，儲存每個本機實體的核准。

### 功能管理員端 {#functional-administrator-side}
