...
## Workspace基本知識 {#workspace-basics}
...
行銷活動元素位於 **[!UICONTROL Campaigns]** 索引標籤：您可以在此處檢視行銷方案、行銷活動及其子集的概觀。 行銷方案是由行銷活動所組成，而行銷活動是由傳遞、任務、連結資源等所組成。 在使用Campaign進行行銷活動管理的情況下，可在行銷活動中找到有關傳送、預算、稽核者和連結檔案的資訊。
...
每個行銷活動都屬於一個方案，而該方案又屬於一個計畫。 計畫、方案和行銷活動可透過 **[!UICONTROL Campaign calendar]** 功能表 **行銷活動** 標籤。

若要編輯計畫、方案、行銷活動或傳遞，請在行事曆中按一下其名稱，然後按一下 **[!UICONTROL Open...]**. 然後會顯示在新標籤中，如下所示：
...
* 此 **排程** 索引標籤會根據您在日曆標題中按一下哪個索引標籤，顯示某月、周或日的計畫行事曆。
...
行銷活動可透過行銷活動行事曆、 **[!UICONTROL Schedule]** 索引標籤或行銷活動清單。
...
1. 透過 **[!UICONTROL Schedule]** 索引標籤上，編輯模式與透過行銷活動行事曆執行的模式相同。
...
對於每個行銷活動，工作、資源和傳遞都會集中在一個畫面（控制面板）中，讓您能夠與其他人共同管理行銷動作。

行銷活動的控制面板會作為控制介面。 它直接存取主要行銷活動建立和管理階段：傳送、擷取檔案、通知、預算等。

![](assets/s_ncs_user_op_board_start_del.png)

透過Adobe Campaign，您可以設定合作流程，以建立和核准行銷和溝通行銷活動的各個階段：核准預算、目標、內容等。
...
行銷活動會集中一組傳遞。 對於每個行銷活動，排程提供所有元件的全域檢視：您可以顯示任務和傳送，並輕鬆存取它們。

![](assets/campaign_planning_tab.png)

#### 論壇 {#forum}

對於每個行銷活動，操作員可以透過專用論壇交換訊息。
...
行銷活動是透過行銷活動範本建立。 您可以設定已選取某些選項且已儲存其他設定的可重複使用範本。 針對每個行銷活動，都會提供下列功能：
...
* 定義成本：對於每個促銷活動，Adobe Campaign可讓您定義 [成本輸入與成本計算結構](../../campaign/using/providers--stocks-and-budgets.md#defining-cost-categories) ，可在建立行銷活動時使用。 例如：印刷成本、使用外部機構、房間租金。
...
* 管理核准：您可以選取要核准的處理方式，並視需要選取稽核操作員或操作員群組。 [了解更多](../../campaign/using/marketing-campaign-approval.md#checking-and-approving-deliveries)

>[!NOTE]
>
>若要存取Campaign設定並進行變更，請按一下 **[!UICONTROL Advanced campaign parameters...]** 中的連結 **[!UICONTROL Edit]** 標籤。
