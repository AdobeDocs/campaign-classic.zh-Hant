...
此外， **行銷資源管理(RM)** 模組可讓您透過提供相關任務、預算和行銷資源的完整管理和即時追蹤，以合作模式控制行銷動作。 「行銷資源管理」可讓您最佳化並規範內部和外部流程、資源和行銷活動的管理，以及第三方關係（代理商、印表機等）。 如需詳細資訊，請參閱[本章節](../../mrm/using/about-marketing-resource-management.md)。

>[!NOTE]
>
>如需Adobe Campaign核心功能的詳細資訊，請參閱 [本節](../../platform/using/about-adobe-campaign-classic.md) 區段。\
>與各種頻道上的母體目標定位、訊息個人化和訊息傳送相關的功能詳見 [本節](../../delivery/using/steps-about-delivery-creation-steps.md).
...
* **工作流程**
   ...
   行銷活動工作流程包含與所有工作流程相同的活動，但特定於行銷活動。 它可讓您為所有可用通道建立和設定傳送。
...
* **傳遞大綱**
   ...
   傳遞大網是傳遞的結構化說明。 每個傳遞都可以參考傳遞大網，其中包含相關優惠方案、要附加的檔案或商店連結。 根據選取的傳遞大網，可在傳遞中參考選件。
