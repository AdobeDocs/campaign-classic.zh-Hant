...
服務供應商設定於 **[!UICONTROL Administration > Campaign management]** 樹狀結構的節點。
...
您必須將服務範本與每個服務提供者建立關聯。 在這些範本中，您必須先識別成本類別，並視需要識別相關存貨。 然後，您必須透過成本結構，為每個分類建立成本計算規則。

>[!NOTE]
>
>有關詳細資訊，請參閱 [定義成本結構](#defining-the-cost-structure).

成本類別是包含一組成本的實體，符合傳送型別（電子郵件、直接郵件等） 或用於任務。 成本類別會分組在與服務提供者相關之服務的範本中。 每個服務提供者可參考一或多個服務範本。

若要建立服務範本並定義其內容，請套用下列步驟：

1. 在 **[!UICONTROL Services]** 標籤中，按一下 **[!UICONTROL Add]** 按鈕並命名服務範本。
...
1. 您可以預先選取此成本類別的值，預設會在服務提供者成本類別中提供此值（而非空白）。 若要這麼做，請選取 **[!UICONTROL Selected]** 相關類別型別的欄：
...
您可以透過以下方式關聯與服務相關聯的程式資訊： **[!UICONTROL Processes]** 標籤。

若要這麼做，請按一下 **[!UICONTROL Processes]** 索引標籤來設定傳送資訊給路由器的方式。
...
您可以在定義傳遞的成本類別時，強制選擇成本型別。 要執行此操作，請選取 **[!UICONTROL A cost type must be selected]**.
...
在行銷活動中，當您選取預算時，必須輸入初始金額。 計算出的成本會根據所輸入金額的履約承諾層次自動更新（費用已產生、預期、預留、已確認）。 另請參閱 [計算金額](../../mrm/using/controlling-costs.md#calculating-amounts).
