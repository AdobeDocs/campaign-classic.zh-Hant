...
product: campaign
title: 異動訊息傳送架構
description: 本節說明Adobe Campaign Classic異動訊息傳送架構，以及傳遞異動訊息的可用通道
...
| 控制例項 | 執行實例 |
|--- |--- |
| Adobe Campaign使用者登入控制執行個體以： <ul><li>建立異動訊息範本</li><li>使用種子清單產生訊息預覽</li><li>顯示報表</li><li>監視執行例項</li></ul> | 執行例項用於： <ul><li>接收事件</li><li>將其連結至異動訊息範本</li><li>傳送個人化訊息給每位收件者</li></ul> |

## 安裝執行個體 {#installing-instances}

安裝異動訊息套件時，有數個預防措施。 Adobe建議您先在測試環境中工作，然後再投入生產。 您還需要有相容的Adobe Campaign授權。 如需詳細資訊，請聯絡您的Adobe客戶主管。

>[!IMPORTANT]
>
>控制執行個體和執行執行個體必須安裝在不同的電腦上。 他們無法共用相同的Campaign執行個體。

如果您需要使用數個通道，必須先安裝並設定相關套件，才能安裝交易式訊息套件。 如需詳細資訊，請參閱 [新增傳遞管道](#adding-a-delivery-channel).

## 控制例項 {#control-instance}

若要在電腦上安裝控制例項，請選取 **[!UICONTROL Transactional message control]** 透過以下方式封裝 **[!UICONTROL Tools]** > **[!UICONTROL Advanced]** > **[!UICONTROL Import package]** 功能表。 如需詳細資訊，請參閱 [安裝Campaign Classic標準套件](../../installation/using/installing-campaign-standard-packages.md).
...
若要在電腦上安裝執行例項，請選取 **[!UICONTROL Transactional message execution]** 透過以下方式封裝 **[!UICONTROL Tools]** > **[!UICONTROL Advanced]** > **[!UICONTROL Import package]** 功能表。 如需詳細資訊，請參閱 [安裝Campaign Classic標準套件](../../installation/using/installing-campaign-standard-packages.md).
...
電子郵件頻道預設為可用。 若要在多個頻道上傳遞交易式訊息，您可以新增其他頻道（行動頻道、行動應用程式頻道等）。

>[!IMPORTANT]
>
>新增傳遞頻道（行動裝置頻道、行動應用程式頻道等） 必須先執行，才能安裝交易式訊息套件。

### 新增傳遞管道 {#adding-a-delivery-channel}

Adobe建議您 **安裝交易式訊息套件之前，請一律新增傳遞通道套件**.

不過，如果您已在電子郵件通道上啟動交易式訊息傳送專案，然後在專案期間決定新增通道，您可以依照下列步驟進行。
...
1. 在 **[!UICONTROL XML content of the data to import]**，僅保留對應至新增頻道的傳送範本。 例如，如果您已新增 **行動裝置頻道**，僅保留 **實體** 與下列專案對應的元素： **[!UICONTROL Mobile transactional message]** (smsTriggerMessage)。 如果您已新增 **行動應用程式頻道**，僅保留 **iOS交易式訊息** (iosTriggerMessage)和 **Android交易式訊息** (androidTriggerMessage)。
...
與行動應用程式頻道模組結合使用時，交易式訊息可讓您透過行動裝置上的通知推送交易式訊息。

>[!NOTE]
>
>行動應用程式頻道的詳細說明，請參閱 [本節](../../delivery/using/about-mobile-app-channel.md).

若要透過行動應用程式頻道使用異動訊息模組，您需要套用以下設定：

1. 安裝 **行動應用程式頻道** 封裝至控制和執行執行個體。
1. 復寫 **行動應用計畫** 輸入Adobe Campaign服務及其包含在執行個體上的行動應用程式。
...
### 交易式訊息傳遞與LINE {#transactional-messaging-and-line}
...
* 安裝 **[!UICONTROL Transactional message control]** 套件進行整合，以及 **[!UICONTROL Transactional message execution]** 執行例項上的套件。
* 建立LINE **外部帳戶** 和 **服務** 在兩個執行個體上，使用相同的命名進行同步處理。 有關如何建立LINE外部帳戶和服務的詳細資訊，請參閱 [本節](../../delivery/using/line-channel.md#setting-up-line-channel).

然後，從 **[!UICONTROL Explorer]** ，在 **[!UICONTROL Platform]** > **[!UICONTROL External account]** ，您必須在兩個執行個體上設定不同的外部帳戶：

1. 建立 **[!UICONTROL External database]** 外部帳戶在您的 **執行** 具有下列設定的執行個體：
...
1. 建立 **[!UICONTROL External Database]** 帳戶在您的 **行銷** 具有下列設定的執行個體。
   ...
   * **[!UICONTROL Server]** ：輸入您行銷活動的執行執行例項伺服器URL。
...
1. 建立 **[!UICONTROL Execution instance]** 外部帳戶在您的 **行銷** 使用下列設定建立資料同步工作流程的執行個體：
   ...
   * **[!UICONTROL Account]** ：輸入用於存取執行個體的帳戶。
   * **[!UICONTROL Password]** ：輸入用來存取執行個體的帳戶密碼。
