...
JavaScript範本由其名稱和名稱空間來識別，就像結構描述和表單一樣。 不過，建議新增 **.js** 範本名稱的選項。

### JavaScript範本的結構 {#structure-of-a-javascript-template}

以&quot;cus：book&quot;結構描述為基礎的JavaScriptHTML格式設定範本範例：
...
* 合併欄位：顯示資料的內容，並附上 **`<%= <source> %>`** 語法，其中 `<source>`是要顯示之資料的來源欄位。
...
下列程式碼會在 `<chapter>` collection element （收集要素）：
...
* **content.chapter.length**：傳回 `<chapter` collection element （收集要素）
* **content.chapter`[0]`.@name**：擷取第一個的名稱 `<chapter>` 元素
...
程式設計語言的所有功能（變數、回圈、條件測試、函式等） )可用來建構輸出檔案。 SOAP API可供存取，以擴充輸出檔案。
...
您可以建構函式或變數程式庫以供稍後使用。 若要這麼做，請使用匯入JavaScript範本 **估計** 函式。 這可讓您使用在其他JavaScript範本中宣告的其他函式來豐富內容。
...
>[!NOTE]
>
>JavaScript物件的初始化必須填入關聯的資料模型結構描述。
...
樣式表是以其名稱和名稱空間來識別，就像結構描述和表單一樣。 不過，建議您新增 **.xsl** 樣式表名稱的延伸。

樣式表的識別鍵是由名稱空間和以冒號分隔的名稱所組成的字串；例如： **cus：book.xsl**.
...
此 **`<xsl:output>`** element指定所產生的檔案格式。 指定所需的字元集和輸出格式。
...
根據預設，XSLT處理器會尋找 **範本** 適用於輸入XML檔案的根或主節點。 輸出檔案的建構會以此開頭 **範本**.

在我們的範例中，藉由顯示書冊名稱和章節清單，從&quot;cus：book&quot;結構描述產生HTML頁面。
...
您可以建立範本或變數的程式庫，以便在多個樣式表之間共用。 &quot;longMonth&quot; **範本**&#x200B;如上所示，是在樣式表中從遠端尋找範本以便日後重複使用之優勢的典型範例。
...
在HTML輸出檔案中輸入的影像可使用絕對或相對參照參照。

相對參照可讓您輸入包含影像的伺服器URL，該影像 **NcmResourcesDir** 和 **NcmResourcesDirPreview** 選項。 這些選項包含在Adobe Campaign使用者端主控台中發佈和預覽的影像位置。
...
在樣式表處理期間， **_resPath** 輸入XML檔案之主要元素上的屬性會根據上下文（預覽或發佈）自動以一個或多個選項填入。
...
>[!NOTE]
>
>我們建議宣告一個變數，其中包含儲存影像的伺服器參考（範例中為「resPath」）。

### 使用公用資源 {#using-public-resources}

您也可以使用 **[!UICONTROL Public resources]** 根據在部署精靈中輸入的執行個體設定，宣告影像並將其上傳到伺服器。
...
在XML輸入檔案中，日期會以內部XML格式儲存： **YYYY/MM/DD HH:MM:SS** (範例2018/10/01 12:23:30)。

Adobe Campaign為JavaScript範本和XSL樣式表提供日期格式功能，詳情如下。

### JavaScript日期格式 {#javascript-date-formatting}

若要以所需格式顯示日期，Adobe Campaign提供 **formatdate** 將日期內容及字串當作輸入的函式，該字串會使用以下語法指定輸出格式： **%4Y/%2M/%2D %2H%2N%2S**

範例:

* 在中顯示日期 **2018年31月10日** 格式：
...
XSLT語法中沒有標準的日期管理函式。 若要以所需格式顯示日期，Adobe Campaign會提供外部功能 **日期格式**. 此函式將日期的內容和字串當作輸入，並指定使用下列語法的輸出格式： **%4Y/%2M/%2D %2H%2N%2S**

範例:

* 若要在中顯示日期 **2018年1月10日** 格式：
