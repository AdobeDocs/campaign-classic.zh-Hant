...
在大多數情況下，主要目標會從Adobe Campaign資料庫（預設模式）中擷取。 不過，收件者也可儲存在外部檔案中。 深入了解 [本節](steps-defining-the-target-population.md#selecting-external-recipients).
...
1. 按一下 **[!UICONTROL Finish]** 來驗證已設定的定位。
