...
此Newsletter包含三種型別的元素：

1. 變數元素，其內容由使用者在建立傳遞期間透過輸入表單輸入或選取。
...
這些元素是透過專用結構描述所建立，該結構描述會為每個內容指定下列元素：名稱、標籤、型別、大小，以及與這些元素在Adobe Campaign中的處理相關的任何其他資訊。

## 步驟2 — 建立資料結構 {#step-2---creating-the-data-schema}

資料結構描述是與內容相關聯的XML檔案。 它說明此內容中資料的XML結構。

>[!NOTE]
>
>如需在Adobe Campaign中建立和設定資料結構的詳細資訊，請參閱 [本節](../../configuration/using/about-schema-edition.md).
>
>有關內容管理專屬的設定元素的詳情，請參閱 [資料結構描述](data-schemas.md).

若要建立資料結構，請套用下列步驟：

1. 開啟Adobe Campaign Explorer並選取 **[!UICONTROL Administration > Configuration > Data schemas]** 節點。

   按一下 **[!UICONTROL New]** 圖示加以識別，並加以標示。
...
>
>有關內容管理專屬的設定元素的詳情，請參閱 [輸入表單](input-forms.md).
...
1. 輸入表單名稱和連結至表單的標籤，然後選取 **[!UICONTROL Content management]** 型別。

   ![](assets/s_ncs_content_param_form_edit.png)

   >[!NOTE]
   >
   >若要讓這兩個元素自動相符，我們建議使用與連結資料結構描述相同的名稱。 使用 **[!UICONTROL Insert]** 按鈕來新增連結至表單的結構描述中的欄位。

   ![](assets/s_ncs_content_param_form_edit_step2.png)

1. 在編輯器的中間部分，指定您想在輸入表單中顯示的欄位。
   ...
   此 **[!UICONTROL Preview]** 索引標籤可讓您在編輯表單時檢查表單的呈現：
...
XSLT語言可讓您將XML檔案轉換為另一個輸出檔案。 在稱為樣式表的檔案中，以XML說明此轉換。

在此範例中，我們想要使用JavaScript範本來定義所產生檔案中的資料建構和版面配置模式。

>[!NOTE]
>
>連結至檔案建置（JavaScript或XSL範本）的限制詳述於 [格式設定](formatting.md).
...
1. 匯入您要在訊息中顯示的設定內容。
   ...
   新增變數元素，同時遵守中詳述的語法 [javascript範本](formatting.md#javascript-templates).
   ...
   在範本開始時呼叫函式，可讓您設定對從Adobe Campaign資料庫擷取的個人化資料的呼叫（在此案例中為： recipient.firstName和recipient.lastName），以便在傳遞中使用它時進行解譯。 有關詳細資訊，請參閱 [包含JavaScript範本](formatting.md#including-a-javascript-template).
   ...
   為了讓JavaScript範本有效，必須先從 **[!UICONTROL JavaScript codes]** 樹狀結構中的節點，如下所示：

   ![](assets/contentmgt_jscode_perso_sample.png)

## 步驟5 — 建立發佈範本 {#step-5---creating-the-publication-template}

下一個步驟包括建立內容發佈範本，以連結結構、表單和內容建構範本。 此出版物範本可以有多種輸出格式。
...
1. 然後輸入範本的名稱，並選擇要套用的演算模式。 在此，我們有一個 **[!UICONTROL JavaScript]** 根據上述建立的範本型別呈現。

   ![](assets/s_ncs_content_param_form_publish.png)

   >[!NOTE]
   >
   >此 **[!UICONTROL DOM interface]** 選項預設為核取，這表示如果您使用E4X語法，將無法存取此檔案。 勾選此選項後必須使用DOM介面，此為建議使用的語法。
   >
...
1. 首先，請透過 **[!UICONTROL Advanced]** 傳遞屬性的索引標籤。
   ...
   傳遞精靈中會新增一個額外索引標籤，以便透過內容管理表單定義內容。

1. 輸入Newsletter的變數資訊。

   ![](assets/s_ncs_content_in_delivery_edition_tab.png)

1. 按一下 **[!UICONTROL HTML preview]** 標籤以檢視演算。 您需要選取收件者以測試個人化。
