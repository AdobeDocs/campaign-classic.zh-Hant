...
Adobe Campaign使您能夠使用 [電子郵件的AMP](https://amp.dev/about/email/) 格式，它允許在特定條件下發送動態電子郵件。

使用AMP for E-mail，您可以：
* Test將AMP電子郵件發送到適當配置的特定地址。
* 在向相應的提供程式註冊後，將AMP電子郵件發送到Gmail、Outlook或Mail.ru地址。
...
目前有三個電子郵件提供商支援測試此格式：Gmail、Outlook和Mail.ru。

在Gmail帳戶上testAMP格式傳送所需的所有步驟和規格在相應的 [Gmail](https://developers.google.com/gmail/ampemail?)。 [展望 ](https://docs.microsoft.com/en-gb/outlook/amphtml/), [郵件.ru](https://postmaster.mail.ru/amp) 開發者文檔。

特別是，必須滿足以下要求：
* 遵循特定於的AMP安全要求 [Gmail](https://developers.google.com/gmail/ampemail/security-requirements)。 [展望](https://docs.microsoft.com/en-gb/outlook/amphtml/security-requirements), [郵件.ru](https://postmaster.mail.ru/amp/?lang=en#howto)。
...
您還可以 [Gmail的提示和已知限制](https://developers.google.com/gmail/ampemail/tips) 和 [AMP Outlook最佳實踐](https://docs.microsoft.com/en-gb/outlook/amphtml/best-practices)。
...
>[!NOTE]
>
>目前只支援測試AMP格式的Gmail、Outlook和Mail.ru。

對於Gmail和Outlook，必須先將您正在使用的發件人地址添加到允許清單中，以便從Adobe Campaign為您目標的Gmail和Outlook帳戶發送郵件。

操作步驟：
1. 確保為相關電子郵件提供程式選中啟用動態電子郵件的選項。
1. 複製交貨中顯示的發件人地址 **[!UICONTROL From]** 欄位並將其貼上到電子郵件提供商帳戶設定的相應部分。

有關詳細資訊，請參閱 [Gmail](https://developers.google.com/gmail/ampemail/testing-dynamic-email) 和 [展望](https://docs.microsoft.com/en-gb/outlook/amphtml/register-outlook#individual-mailbox-registration) 開發者文檔。

![](assets/amp_from_field.png)

要test將AMP電子郵件發送到Mail.ru地址，請按照 [Mail.ru開發人文檔](https://postmaster.mail.ru/amp/?lang=en#howto) (**如果您是用戶** )的正平方根。
...
使用幾個地址測試後，您可以向任何Gmail或Outlook地址發送AMP電子郵件。 為此，你必須恭敬地向Google或Microsoft註冊，並等待他們的答復。 按照中介紹的步驟操作 [Gmail](https://developers.google.com/gmail/ampemail/register) 和 [展望](https://docs.microsoft.com/en-gb/outlook/amphtml/register-outlook#global-registration) 開發者文檔。 註冊成功後，您將成為授權發件人。

要將AMP電子郵件發送到Mail.ru地址，請遵循中列出的要求和步驟 [Mail.ru開發人文檔](https://postmaster.mail.ru/amp/?lang=en#howto) (**如果您是電子郵件發件人** )的正平方根。

## 發送AMP電子郵件 {#sending-amp-email}

一旦AMP內容和備用準備就緒，並且定義了相容目標後，您就可以像通常一樣發送電子郵件。

目前，在某些條件下，只有Gmail、Outlook和Mail.ru支援AMP格式。 您可以瞄準來自其他電子郵件提供商的地址，但他們將接收您的電子郵件的HTML或純文字檔案版本。
