...
沒有。在執行個體升級後，您的電子郵件傳遞只能使用Enhanced MTA。
...
沒有。升級不需要切換至新IP，因此您可以繼續使用現有的熱電子郵件IP。
...
>[!NOTE]
>
>使用 [電子郵件意見回饋服務](#email-feedback-service) (EFS)功能（目前以測試版形式提供）, 「促銷活動傳送」輸送量圖表仍會顯示電子郵件收件者的輸送量。

### 重試次數

Campaign不再使用傳送中的重試設定。 軟退信重試次數及兩者之間的時間長度，由Enhanced MTA根據來自訊息電子郵件網域傳回之退信的類型和嚴重性決定。
