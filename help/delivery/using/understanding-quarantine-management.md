...
* 位於 **封鎖清單**&#x200B;另一方面，將會導致 **設定檔** 不再被傳送定位，例如在針對指定頻道取消訂閱（選擇退出）後。 例如，如果電子郵件頻道封鎖清單上的設定檔有兩個電子郵件地址，則兩個地址都會從傳送中排除。

  您可以檢查設定檔是否位於一或多個頻道的封鎖清單中 **[!UICONTROL No longer contact]** 設定檔的區段 **[!UICONTROL General]** 標籤。 請參閱[本節](../../platform/using/editing-a-profile.md#general-tab)。

>[!NOTE]
>
>隔離包括 **[!UICONTROL Denylisted]** 狀態，當收件者將您的訊息回報為垃圾訊息，或回覆具有關鍵字（例如「STOP」）的SMS訊息時套用。 在這種情況下，設定檔的相關地址或電話號碼會傳送到隔離區，並包含 **[!UICONTROL Denylisted]** 狀態。 有關管理STOP SMS訊息的詳細資訊，請參閱 [本節](../../delivery/using/sms-send.md#processing-inbound-messages).
...
在傳送準備階段期間，會將特定傳送的隔離地址列在傳送控制面板的傳送記錄中(請參閱 [傳遞記錄和歷史記錄](delivery-dashboard.md#delivery-logs-and-history))。

### 識別整個平台的隔離地址 {#identifying-quarantined-addresses-for-the-entire-platform}

管理員可以從以下位置列出整個平台之隔離區中的地址： **[!UICONTROL Administration > Campaign Management > Non deliverables Management > Non deliverables and addresses]** 節點。
...
* 此 **[!UICONTROL Non-deliverables and bounces]** 報告會顯示隔離地址、遇到的錯誤型別等資訊，以及依網域劃分的失敗資訊。
...
重試將執行於 [傳遞期間](../../delivery/using/steps-sending-the-delivery.md#defining-validity-period). 當錯誤計數器達到限制臨界值時，該地址就會進入隔離區。有關詳細資訊，請參閱 [傳送暫時失敗後重試](understanding-delivery-failures.md#retries-after-a-delivery-temporary-failure).

如果最後一次重大錯誤發生在10天以前，則會重新初始化錯誤計數器。 然後，地址狀態會變更為 **有效** 並且會由將其從隔離區清單中刪除 [資料庫清理](../../production/using/database-cleanup-workflow.md) 工作流程。


對於託管或混合安裝，如果您已升級至 [增強的MTA](sending-with-enhanced-mta.md)，在下列情況下要執行的最大重試次數： **[!UICONTROL Erroneous]** 狀態以及重試之間的最小延遲現在取決於IP在歷史和目前指定網域的執行狀況。

對於使用舊版Campaign MTA的內部部署安裝和託管/混合安裝，您可以修改錯誤數量和兩個錯誤之間的期間。 若要這麼做，請變更 [部署精靈](../../installation/using/deploying-an-instance.md) (**[!UICONTROL Email channel]** > **[!UICONTROL Advanced parameters]**)或 [在傳遞層級](../../delivery/using/steps-sending-the-delivery.md#configuring-retries).
...
SMS訊息的隔離機制與一般程式在整體上相同。 另請參閱 [關於隔離](#about-quarantines). 簡訊的特定性列於下方。
...
使用SMPP通訊協定傳送SMS訊息時，錯誤管理的處理方式不同。 有關擴展的通用SMPP聯結器的詳細資訊，請參閱 [此頁面](sms-set-up.md#creating-an-smpp-external-account).
...
* 第二部分(**通用** 在此範例中，錯誤訊息會參照SMSC實作的名稱，例如 **[!UICONTROL SMSC implementation name]** 簡訊外部帳戶的欄位。 請參閱[此頁面](sms-set-up.md#creating-an-smpp-external-account)。
...
* 管路符號(|)後面的所有專案只會顯示在 **[!UICONTROL First text]** 的欄 **[!UICONTROL Delivery log qualification]** 表格。 此內容一律會取代為 **#MESSAGE#** 在訊息標準化之後。 此程式會避免因類似錯誤而出現多個專案，與電子郵件的情況相同。 有關詳細資訊，請參閱 [退回郵件資格](understanding-delivery-failures.md#bounce-mail-qualification).
