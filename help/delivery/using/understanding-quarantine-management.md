...
### 在傳送報告中標識隔離地址{#identifying-quarantined-addresses-in-delivery-reports}
...
### 標識收件者的隔離地址{#identifying-quarantined-addresses-for-a-recipient}
...
* 裝載長度問題：無重試，故障原因為&#x200B;**[!UICONTROL Unreachable]**。
* 憑證到期問題：無重試，故障原因為&#x200B;**[!UICONTROL Unreachable]**。
...
HTTP/V2通訊協定可讓每個推播傳送直接提供意見和狀態。 如果使用HTTP/V2通訊協定連接器，**[!UICONTROL mobileAppOptOutMgt]**&#x200B;工作流程將不再呼叫回饋服務。 未註冊的Token在iOS二進位連接器和iOS HTTP/V2連接器之間的處理方式不同。 解除安裝或重新安裝行動應用程式時，會將Token視為未註冊。
