...
>[!NOTE]
>
>**data_coding** 值及實際使用編碼之間的對應是標準化的。然而，特定SMSC有其獨特的對應：在這種情況下，您的 **Adobe Campaign** 管理員需要宣告此對應。 請洽詢您的提供者以瞭解更多資訊。

您可以宣告 **data_codings** 並視需要強制進行編碼：若要這麼做，請在表格中指定單一編碼。
...
* 當您定義要使用的編碼以及連結的編碼時 **[!UICONTROL data_coding]** 欄位值，Adobe Campaign會嘗試使用清單中的第一個編碼，如果第一個編碼不可能，則請執行下列操作。
...
當訂閱者回覆透過Adobe Campaign傳送給他們的簡訊訊息，且訊息包含「STOP」之類的關鍵字時，您可以設定在 **[!UICONTROL Automatic reply sent to the MO]** 區段。
...
* 若要無論簡碼如何都傳送相同的回覆，請將 **[!UICONTROL Short code]** 欄空白。
* 無論關鍵字為何，若要傳送相同的回覆，請將 **[!UICONTROL Keyword]** 欄空白。
* 若要在不傳送回應的情況下執行動作，請保留 **[!UICONTROL Response]** 欄空白。 例如，這可讓您從隔離區中移除回複訊息不是「STOP」的使用者。
...
* 為每個外部帳戶建立一個提供者帳戶。
* 使用 **[!UICONTROL System type]** 欄位來自 **[!UICONTROL Mobile]** > **[!UICONTROL Connection settings]** 標籤以區分每個簡短代碼。 請向您的提供者詢問每個帳戶的不同值。

  ![](assets/extended_smpp_system-type.png)

有關使用擴充通用SMPP聯結器設定外部帳戶的詳細步驟，請參見 [建立SMPP外部帳戶](#creating-an-smpp-external-account) 區段。
...
1. 在 **[!UICONTROL General]** 標籤，選取與先前步驟中建立的外部帳戶對應的路由模式。
