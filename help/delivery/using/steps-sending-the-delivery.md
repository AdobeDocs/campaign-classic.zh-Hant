...
要執行此操作，請按一下&#x200B;**[!UICONTROL Send]**，選擇所需的操作，然後按一下&#x200B;**[!UICONTROL Analyze]**。 有關詳細資訊，請參閱[啟動分析](steps-validating-the-delivery.md#analyzing-the-delivery)。
...
1. 要配置波，可以執行以下任一操作：
   ...
   * 定義傳送每個波次的日曆。
   ...
   特定的類型規則&#x200B;**[!UICONTROL Wave scheduling check]**&#x200B;可確保在傳送有效性限制之前已規劃最後一個波次。 在傳送屬性的&#x200B;**[!UICONTROL Typology]**&#x200B;標籤中設定的促銷活動類型及其規則，會顯示在包含類型](steps-validating-the-delivery.md#validation-process-with-typologies)的驗證程式中。[
...
由於&#x200B;**Soft**&#x200B;或&#x200B;**Ignored**&#x200B;錯誤而暫時未傳送的郵件可能會自動重試。 傳遞失敗類型和原因見此[節](understanding-delivery-failures.md#delivery-failure-types-and-reasons)。
...
* **[!UICONTROL Delivery duration]**&#x200B;欄位可讓您輸入全域傳送重試的限制。 這表示Adobe Campaign會傳送從開始日期開始的訊息，然後，對於僅傳回錯誤的訊息，會執行一般、可設定的重試，直到達到有效性限制為止。
   ...
   >[!IMPORTANT]
   >
   >若為托管或混合安裝，如果您已升級至[Enhanced MTA](sending-with-enhanced-mta.md)，則只有在設為&#x200B;**3.5天或更短時間**&#x200B;時，才會使用Campaign電子郵件傳送中的&#x200B;**[!UICONTROL Delivery duration]**&#x200B;設定。  如果您定義的值超過　3.5　天，則不會考慮該值。
