...
>[!IMPORTANT]
>
>為了 **[!UICONTROL Database cleanup]** 要在調度程式中定義的日期和時間啟動工作流，必須啟動工作流引擎(wfserver)。 如果情況並非如此，則直到下次啟動工作流引擎時才會進行資料庫清理。
...
1. 此 **[!UICONTROL Database cleanup]** 工作流程會建立已過期的傳送清單。 此清單包含狀態為非 **[!UICONTROL Finished]** ，以及最近停止傳送超過10,000個未處理的訊息。 使用下列查詢：
   ...
   where **傳遞模式1** 符合 **[!UICONTROL Mass delivery]** 模式， **州51** 符合 **[!UICONTROL Start pending]** 州， **州85** 符合 **[!UICONTROL Stopped]** 狀態，且傳送伺服器上大量更新的傳送記錄數最多等於10,000。
...
1. 在過期傳遞的清單中，狀態為 **[!UICONTROL Pending]** ，切換至 **[!UICONTROL Delivery cancelled]** ，此清單中的所有傳送會切換至 **[!UICONTROL Finished]** .
   ...
   where **$(curdate)** 是資料庫伺服器的當前日期， **$(bl)** 是傳送記錄檔訊息的識別碼， **$(dl)** 是傳遞識別碼， **傳遞狀態6** 符合 **[!UICONTROL Pending]** 狀態與 **傳遞狀態7** 符合 **[!UICONTROL Delivery cancelled]** 狀態。

   ```
   UPDATE NmsDelivery SET iState = 95, tsLastModified = $(curdate), tsBroadEnd = tsValidity WHERE iDeliveryId = $(dl)
   ```

   where **傳送狀態95** 符合 **[!UICONTROL Finished]** 狀態，和 **$(dl)** 是傳送的識別碼。
...
1. 所有片段(**deliveryParts**)，並刪除所有進行中通知傳送的過時片段。 這兩個任務都使用大量刪除。
   ...
   where **傳送狀態95** 符合 **[!UICONTROL Finished]** 狀態， **傳送狀態85** 符合 **[!UICONTROL Stopped]** 狀態，和 **$(curDate)** 是目前的伺服器日期。
...
1. 接著會排除進行中的工作流程所使用的表格。 要執行此操作，系統會使用下列查詢來復原進行中的傳送清單：
   ...
   其中0是符合 **[!UICONTROL Being edited]** 傳送狀態，85符合 **[!UICONTROL Stopped]** 狀態和100符合 **[!UICONTROL Deleted]** 狀態。
...
where **狀態2** 符合 **[!UICONTROL Valid]** 狀態， **$(tsDate1)** 是目前的伺服器日期，和 **$(tsDate2)** 符合 **NmsCleanup_LastCleanup** 選項。
