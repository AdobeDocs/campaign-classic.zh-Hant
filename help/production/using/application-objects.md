...
匯入工作流程會建立檔案，因此造成磁碟擴充。 使用標準[檔案收集器](../../workflow/using/file-collector.md)活動可避免此問題。 檔案收集器會將檔案移至暫存資料夾，並自動將其清除。

如果工作流程匯入檔案但未使用標準功能，則需要清除該工作流程以將磁碟空間維持在最低限度。

## 異動資料和記錄 {#transactional-data-and-logs}

每[工作流程](../../workflow/using/data-life-cycle.md#work-table)將資料匯入Adobe Campaign時，會導致資料庫的大小增加。
