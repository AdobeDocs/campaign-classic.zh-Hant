...
運運算元會從主控台中的設定檔連結至安全性區域，可在中存取 **[!UICONTROL Administration > Access management > Operators]** 節點。 [了解更多](#linking-a-security-zone-to-an-operator)。

>[!NOTE]
>
>此程式僅限於 **內部部署** 部署。
>
>作為 **託管** 客戶，如果您可存取 [Campaign控制面板](https://experienceleague.adobe.com/docs/control-panel/using/control-panel-home.html?lang=zh-Hant)，您可以使用Security Zone自助服務介面。 [了解更多](https://experienceleague.adobe.com/docs/control-panel/using/instances-settings/ip-allow-listing-instance-access.html?lang=zh-Hant)
>
...
區域必須在伺服器組態期間定義，在 **serverConf.xml** 檔案。 所有引數都可在 **serverConf.xml** 列於 [本節](../../installation/using/the-server-configuration-file.md).
...
>[!NOTE]
>
>**每個運運算元都必須連結至區域**. 如果運運算元的IP位址屬於區域定義的範圍，則運運算元可登入執行個體。\
>操作員的IP位址可以在多個區域中定義。 在此案例中，運運算元接收 **設定** 每個區域的可用許可權數量。
...
* **allowEmptyPassword**：授權使用密碼連線至執行個體
...
使用訊息中心時，如果有數個執行個體，您需要使用建立額外的安全性區域 **sessionTokenOnly** 屬性定義為 **true**，僅新增必要的IP位址。 有關設定執行個體的詳細資訊，請參閱 [本檔案](../../message-center/using/configuring-instances.md).

## 安全性區域的最佳作法 {#best-practices-for-security-zones}

在「 」的定義 **區域網路** 安全區域，可以新增定義技術存取的IP位址遮罩。 此新增將可讓您存取伺服器上裝載的所有執行個體。
...
此 **proxy** 引數可以用在 **子網路** 元素來指定安全區域中的Proxy使用。
...
可能存取Adobe Campaign伺服器的代理程式IP位址必須同時輸入兩項 **`<subnetwork>`** 相關和第一層子網路 **`<subnetwork name="all"/>`**. 例如，此處針對IP位址為10.131.146.102的Proxy：
...
在此之前，您必須從設定現成可用的開始 **[!UICONTROL Security zone]** 將標籤連結至中定義之區域的內部名稱的列舉 **serverConf.xml** 檔案。
...
1. 在 **[!UICONTROL Internal name]** 欄位中，輸入 **serverConf.xml** 檔案。 它對應至 **@name** 的屬性 `<securityzone>`  元素。 輸入連結到內部名稱的標籤，在  **標籤**&#x200B;欄位。

   ![](assets/enum_addsecurityvalue.png)

1. 按一下「確定」並儲存修改。

定義區域後，以及 **[!UICONTROL Security zone]** 已設定分項清單，您必須將每個運運算元連結至安全性區域：

1. 按一下 **[!UICONTROL Administration > Access management > Operators]** 節點。
1. 選取您要連結安全區域的運運算元，然後按一下 **[!UICONTROL Edit]** 標籤。
1. 前往 **[!UICONTROL Access rights]** 標籤並按一下 **[!UICONTROL Edit access parameters...]** 連結。

   ![](assets/zone_operator.png)

1. 從以下位置選取區域 **[!UICONTROL Authorized connection zone]** 下拉式清單
...
* 請確定您的反向Proxy不允許出現在subNetwork中。 如果是這種情況， **全部** 系統將偵測到來自此本機IP的流量，因此信任該流量。

* 儘量不使用sessionTokenOnly=&quot;true&quot;：

   * 警告：如果這個屬性設定為true，運運算元就可以顯示為 **CRSF攻擊**.
   * 此外，sessionToken Cookie未設定httpOnly標幟，因此某些使用者端JavaScript程式碼可以讀取。
   * 不過，多個執行儲存格上的Message Center需要sessionTokenOnly：在sessionTokenOnly設為&quot;true&quot;的情況下建立新的安全性區域，並新增 **僅限所需的IP** 在此區域中。
...
* 切勿將allowEmptyPassword、allowUserPassword、allowSQLInjection設定為true。 這些屬性僅供從v5和v6.0順利移轉之用：
   ...
   * **allowUserPassword** 可讓操作員傳送其認證作為引數（以便由apache/IIS/proxy記錄）。 此功能過去曾用於簡化API的使用。 無論某些協力廠商應用程式是否使用此功能，您都可以簽入您的逐步指南（或規格）。 若是如此，您必須通知他們變更使用API的方式，並儘快移除此功能。
