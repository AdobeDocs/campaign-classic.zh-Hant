...
* HTTP或TCP負載平衡器後面的專用重新導向伺服器，用於擴充性和可用性，
...
使用這類設定時，透過適當的頻寬和調整，每小時的預期傳送量可能超過100,000封郵件。

## 功能 {#features}

### 優點 {#advantages}

* 最佳化安全性：只有需要對外公開存取的伺服器才會安裝在DMZ的電腦上。
* 高可用性更易於確保：只有從外部可見的電腦才需要以高可用性來管理。
...
>[!CAUTION]
>
>如果您的應用程式伺服器指向單一資料庫執行個體，則在一個執行個體上匯入標準套件後，封裝中包含的結構描述不會在其他執行個體上載入。
>  
>如果您的應用程式伺服器指向單一資料庫執行個體，則變更一個執行個體上的結構描述後，不會在其他執行個體上載入該結構描述。
>
>若要復原這些問題，您必須在發生錯誤的第二個執行個體上重新開機&#39;web@default&#39;程式。

### 安裝及設定應用程式伺服器1 {#installing-and-configuring-the-application-server-1}
...
1. 編輯 **config-demo.xml** 檔案(透過上一個命令建立，位於 **config-default.xml** 檔案)，檢查 **mta** （傳遞）， **wfserver** （工作流程）， **inMail** （反彈郵件）和 **stat** (statistics)處理序已啟用，然後設定 **app** 統計值伺服器：
...
1. 測試 **nlserver web** 模組使用URL： [https://console.campaign.net/nl/jsp/logon.jsp](https://tracking.campaign.net/r/test).
   ...
   此URL可讓您存取使用者端設定程式的下載頁面。 [了解更多](../../installation/using/client-console-availability-for-windows.md)。

   輸入 **內部** 存取控制頁面時的登入及相關密碼。

   ![](assets/s_ncs_install_access_client.png)

### 安裝及設定應用程式伺服器2 {#installing-and-configuring-the-application-server-2}
...
1. 安裝Adobe Campaign伺服器，
1. 請遵循以下各節中所述的Web伺服器整合程式(IIS、Apache)：
...
1. 複製 **config-demo.xml** 和 **serverConf.xml** 在安裝期間建立的檔案。 在 **config-demo.xml** 檔案，啟動 **trackinglogd** 處理和停用 **mta**， **inmail**， **wfserver** 和 **stat** 程式。
1. 編輯 **serverConf.xml** 檔案並在重新導向的引數中填入多餘的追蹤伺服器：
