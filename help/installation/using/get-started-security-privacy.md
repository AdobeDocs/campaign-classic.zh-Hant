...
本節將向您介紹有關安全性和隱私權需要檢查的關鍵元素。 部分設定只能由內部部署客戶執行。
...
* **在網頁應用程式中新增字幕**：瞭解如何在您的公開登陸頁面和訂閱頁面中新增驗證碼。
...
在部署內部部署型別的架構時，需要檢查的一件非常重要的事情是網路設定。
...
>[!CAUTION]
>
>自2021年7月14日起，任何不支援TLS 1.2通訊協定的使用者端系統將無法存取所有Adobe產品和服務。 在此日期之前，請確定所有使用者和使用者端系統都符合TLS 1.2。 [了解更多](https://helpx.adobe.com/x-productkb/multi/eol-tls-support.html)
