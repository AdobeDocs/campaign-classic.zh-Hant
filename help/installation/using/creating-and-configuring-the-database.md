...
1. 使用現有資料庫：如果管理員已建立空白資料庫且您想要使用它，請選擇此選項；或擴充現有資料庫的結構。 請參閱 [案例2：使用現有資料庫](#case-2--using-an-existing-database).
...
在 **[!UICONTROL Server access]** 視窗，定義資料庫伺服器存取。
...
* 對於Oracle、Microsoft SQL Server或PostgreSQL資料庫，管理員也可以定義 **儲存引數** 建立資料庫物件時使用。
  ...
  這些引數會收到完全相同的表格空間名稱（警告：區分大小寫）。 它們會分別儲存在 **[!UICONTROL Administration > Platform > Options]** 節點(請參閱 [本節](../../installation/using/configuring-campaign-options.md#database))：

   * **WdbcOptions_TableSpaceUser**：根據綱要的使用者表格
   * **WdbcOptions_TableSpaceIndex**：根據綱要的使用者表格索引
...
* 此 **[!UICONTROL Set or change the administrator password]** 選項可讓您輸入連結至Adobe Campaign運運算元（具有管理員許可權）的密碼。
