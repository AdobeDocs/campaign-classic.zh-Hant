...
product: campaign
title: 設定Netezza的存取權
description: 瞭解如何在FDA中設定Netezza的存取權
...
1. 根據您使用的作業系統，安裝適用於Netezza的ODBC驅動程式：
...
1. 指定Adobe Campaign伺服器的環境變數：
   ...
   * **NZ_ODBC_INI_PATH**： odbc.ini檔案的位置。 Netezza也需要第二個變數才能使用odbc.ini檔案。
...
>
>資料表將在結構描述中定義的第一個索引上使用&#x200B;**Organize on**&#x200B;子句。 由於此子句在Netezza中限製為1到4個資料行，因此此索引不能包含超過4個資料行。
