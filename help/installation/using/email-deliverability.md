...
>[!NOTE]
>
>某些設定只能透過Adobe來執行，適用於由Adobe托管的部署，例如，存取伺服器和執行個體設定檔。 若要進一步了解不同部署，請參閱[托管模型](../../installation/using/hosting-models.md)區段或[此頁面](../../installation/using/capability-matrix.md)。

如需與Adobe Campaign傳遞能力相關的概念和最佳實務，請參閱此[區段](../../delivery/using/about-deliverability.md)。
...
您可以控制一或多個Adobe Campaign例項的輸出，以根據網域限制傳送的電子郵件數量。 例如，您可以將&#x200B;**yahoo.com**&#x200B;位址的輸出限制為每小時20,000條，同時為所有其他網域設定每小時100,000條訊息。

需要針對傳送伺服器使用的每個IP位址來控制訊息輸出(**mta**)。 多部&#x200B;**mta**&#x200B;分割成多部電腦，且屬於各種Adobe Campaign例項，可共用相同的IP位址以進行電子郵件傳送：需要設定程式來協調這些IP位址的使用。

以下是&#x200B;**stat**&#x200B;模組的功能：它會針對一組IP地址將所有連接請求和郵件轉發到郵件伺服器。 統計伺服器可跟蹤傳送，並可根據設定的配額啟用或禁用發送。
...
1. **mta**&#x200B;選擇符合條件的消息，並為它們分配一個可用的&#x200B;**mtachild**。
1. **mtachild**&#x200B;會載入建立訊息（內容、個人化元素、附件、影像等）所需的所有資訊 並將消息轉發到&#x200B;**電子郵件流量Shaper**。
1. 當電子郵件流量整形器收到統計伺服器的授權(**smtp stat**)時，就會將郵件發送給收件人。
...
1. **會話失敗** （5.11以上）:如果 **** mta收到此訊息的答案，訊息會捨棄(請參閱 [訊息放棄](#message-abandonment))。如果沒有其他路徑可用，則將消息發送到其他路徑或設定為掛起（請參閱[消息掛起](#message-pending)）。
...
對於此記錄，用戶可以聯繫8個對等IP地址。 由於他有2個公開IP位址，因此他8 * 2 = 16組合即可連線至yahoo.com郵件伺服器。 這些組合中的每一個都稱為路徑。
...
如果&#x200B;**[!UICONTROL MX management]**&#x200B;文檔在節點中不存在，可以手動建立它。 操作步驟：
...
* **[!UICONTROL MX mask]**:套用規則的網域。每個規則都定義MX的地址掩碼。 因此，任何名稱與此掩碼匹配的MX都符合條件。 掩碼可以包含「*」和「？」 一般字元。
   ...
   * ?.mx.yahoo.com
   ...
   在這種情況下，將使用MX規則`*.google.com`。 如您所見，MX規則遮罩不一定符合郵件中的網域。 套用至gmail.com電子郵件地址的MX規則將是遮罩`*.google.com`的規則。
...
* **[!UICONTROL Connection time out]**:連接到域的時間閾值。
   ...
   >[!NOTE]
   >
   >此臨界值之前，Windows可能會發出&#x200B;**timeout**，這取決於您的Windows版本。
...
* **[!UICONTROL TLS]**:可讓您加密電子郵件傳送的TLS通訊協定可選擇性地啟用。對於每個MX掩碼，可使用以下選項：
   ...
   * **[!UICONTROL Opportunistic]** :如果接收伺服器(SMTP)可產生TLS通訊協定，則會加密訊息傳送。
...
要執行此操作，請轉到位於&#x200B;**[!UICONTROL Administration]** > **[!UICONTROL Campaign management]** > **[!UICONTROL Non deliverables management]** > **[!UICONTROL Mail rule sets]**&#x200B;的&#x200B;**[!UICONTROL Management of email formats]**&#x200B;文檔。

本檔案包含與Adobe Campaign管理的日文格式對應的所有預先定義網域清單。 有關詳細資訊，請參閱[本文檔](../../delivery/using/defining-the-email-content.md#sending-emails-on-japanese-mobiles)。

![](assets/mail_rule_sets.png)

**MIME結構**（多用途Internet郵件副檔名）參數允許您定義將發送到不同郵件客戶端的郵件結構。 有三種可用選項：

* **多部分**:訊息會以文字或HTML格式傳送。如果不接受HTML格式，訊息仍能以文字格式顯示。

   依預設，多部分結構為&#x200B;**多部分/替代**，但當影像新增至訊息時，它會自動變成&#x200B;**多部分/相關**。 某些提供者預設會使用&#x200B;**multipart/related**&#x200B;格式，即使未附加影像，**[!UICONTROL Force multipart/related]**&#x200B;選項也會實施此格式。

* **HTML**:僅傳送HTML訊息。如果不接受HTML格式，則不會顯示訊息。
* **文字**:只會傳送文字格式的訊息。文本格式消息的優點是其大小非常小。

如果啟用&#x200B;**[!UICONTROL Image inclusion]**&#x200B;選項，這些選項會直接顯示在電子郵件的正文中。 接著會上傳影像，而URL連結將會被其內容取代。

日本市場對&#x200B;**Deco-mail**、**Decore Mail**&#x200B;或&#x200B;**Decore Mail**&#x200B;特別使用此選項。 有關詳細資訊，請參閱[本文檔](../../delivery/using/defining-the-email-content.md#sending-emails-on-japanese-mobiles)。
...
配置的&#x200B;**mta**&#x200B;元素的&#x200B;**statServerAddress**&#x200B;屬性允許您指定要使用的埠的地址和數量。
...
要在同一台電腦上使用統計伺服器，至少必須輸入具有&#x200B;**localhost**&#x200B;值的電腦名稱：
...
對於每個&#x200B;**IPAffinity**&#x200B;元素，需要聲明可用於電腦的IP地址。
...
* **publicId**:當NAT路由器後面的幾個Adobe Campaignmtas共用IP地址時， **** 此資訊很有用。統計伺服器使用該標識符來儲存該起始點和目標伺服器之間的連接併發送統計資訊。
...
* **excludeDomains**:不包括此IP位址的網域清單。此篩選器會在&#x200B;**includeDomains**&#x200B;篩選器之後套用。
...
達到&#x200B;**maxWorkingSetMb**(256)臨界值後，傳送伺服器就停止傳送訊息。 效能會大幅下降，直到&#x200B;**mtachild**&#x200B;重新啟動為止。 要避免此問題，您可以增加&#x200B;**maxWorkingSetMb**&#x200B;參數的閾值，或降低&#x200B;**maxWaitingMessages**&#x200B;參數的閾值。
