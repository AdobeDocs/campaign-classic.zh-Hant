...
在移轉期間， **NmsRecipient** 表格會從結構描述定義重新建置。 在Adobe Campaign外部對此表格的SQL結構所做的任何變更都將遺失。

要檢查的元素範例：

* 如果您已新增欄（或索引）至 **NmsRecipient** 但您尚未在結構描述中詳細說明該表格，則不會儲存此表格。
* 此 **表格空間** 屬性預設會取回其值，換言之，會取回部署精靈中定義的值。
...
* 鏡像頁面
  ...
  不過，作為臨時因應措施（且映象頁面仍在使用中），您可以返回舊的個人化區塊，透過變更選項來避免此問題 **[!UICONTROL XtkAcceptOldPasswords]** 並將其設為 **[!UICONTROL 1]**. 這不會影響新v6.x個人化區塊的使用。
...
* 報告
  ...
  目前所有現成可用的報表都使用v6.x轉譯引擎。 如果您已將JavaScript程式碼新增至報表中，某些元素可能會受到影響。
