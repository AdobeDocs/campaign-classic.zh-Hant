...
使用者端與伺服器之間的時區不符也可能會造成一些延遲。 因此，我們建議在使用者端和伺服器端使用相同版本的Oracle程式庫，且兩個時區必須相同。
...
-->
如果您希望從新的報告功能中獲益，則必須重新發佈報告。 在此情況下，請檢查所有指令碼，並視需要加以變更。 關於PDF匯出，如果您已針對Open Office新增特定指令碼，則新PDF匯出引擎(PhantomJS)將無法再使用此功能。
