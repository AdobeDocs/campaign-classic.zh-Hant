...
共有四種類型 `<element>`  Adobe Campaign中的元素：
...
* **匯總（字串）**:可讓您複製 `<element>`  透過其他架構。 此屬性會以「namespace:name」的形式接收架構聲明。
* **applicatedIf（字串）**:條件。 此屬性會接收XTK運算式。
...
* **displayAsField（布林值）**:如果此屬性已啟用，則為「連結」類型 `<element>`  將顯示為結構樹視圖（「結構」頁簽）中的欄位。 這樣，可將連結顯示為本機欄位，並在查詢期間變更其行為。 在查詢的SELECT中找到元素時，將使用連結目標的值。 在查詢的WHERE中找到元素時，將會使用連結的基礎索引鍵。
...
* **length（字串）**:max。 為「字串」類型SQL欄位的值授權的字元數。
...
* **有序（布林值）**:如果屬性已啟用(ordered=&quot;true&quot;),Adobe Campaign會將元素聲明序列保留在XML收集元素中。
* **pkSequence（字串）**:接收用於計算自動增量密鑰的序列的名稱。 只有在架構的根元素上定義了自動增量鍵時，才可使用此屬性。
...
* **revCardinality（字串）**:此屬性定義兩個表之間連結的基數。 用於「連結」類型 `<element>`.
...
* **revLabel（字串）**:標籤。
* **revLink（字串）**:相對連結的名稱。 如果值為「_無_「，目標架構中不會建立相反的連結。
* **revTarget（字串）**:相對連結的目標。
* **sql（布林值）**:如果激活了此屬性(@sql=&quot;true&quot;)，則會強制儲存SQL元素，即使元素具有xml=&quot;true&quot;屬性亦然。
* **sqlname（字串）**:表建立期間的欄位名稱。 如果未指定&quot;@sqlname&quot;，預設會使用&quot;@name&quot;屬性的值。 將架構寫入表時，會根據欄位類型自動添加前置詞。
* **sqltable（字串）**:對於架構的主要元素，此屬性將預設生成的SQL表的名稱重載。 如果未指定「@sqltable」，則預設名稱的結構將如下所示：namespace（大寫字母），後跟SrcSchema的值「@name」。
* **tableSpace(string)**:通過此屬性，可以指定儲存表的表空間的新資料（在根上有效） `<element>`)。
* **tableSpaceIndex(string)**:通過此屬性，可以為表指定新的索引儲存表空間（在根上有效） `<element>`)。
* **target(MNTOKEN)**:在表之間建立連結時接收目標架構的名稱。 此屬性僅對「連結」類型元素作用中。
* **範本（字串）**:此屬性定義對 `<element>` 由數個結構共用的元素。 定義會自動複製到目前的架構中。
...
* **xmlChildren（布林值）**:強制儲存每個子代( `<element>  or  <attribute>   ) of the   <element>    element in an XML document.   </element>  </attribute> </element>`
