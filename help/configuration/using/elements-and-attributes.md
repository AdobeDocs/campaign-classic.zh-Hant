...
* **appliableIf（字串）**: 條件，用於考慮索引——接收XTK表達式。
* **標籤（字串）**: 索引標籤。
* **名稱(MNTOKEN)**: 唯一索引名。
...
* **名稱(MNTOKEN)**: 唯一方法名稱。
* **靜態（布林值）**: 如果激活了此屬性，則該方法被視為自治，在調用該屬性時，必須將所有參數指定給該方法。
