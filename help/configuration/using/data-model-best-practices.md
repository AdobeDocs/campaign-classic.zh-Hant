...
如需Campaign內建表格及其互動的詳細資訊，請參閱 [本節](../../configuration/using/about-data-model.md) 區段。

閱讀 [本檔案](../../configuration/using/about-schema-reference.md) 以開始使用Campaign綱要。 瞭解如何設定擴充功能綱要，以擴充中的Adobe Campaign資料庫概念資料模型 [本檔案](../../configuration/using/about-schema-edition.md).
...
Adobe Campaign預設資料模型顯示於 [本檔案](../../configuration/using/data-model-description.md).

>[!NOTE]
>
>Adobe Campaign Classic允許建立自訂客戶表格。 但在大多數情況下，建議採用此標準 [收件者表格](../../configuration/using/about-data-model.md#default-recipient-table) 已預先建立其他表格和功能。
...
* 大型表格通常應該有數值欄位，並包含參考表格的連結（使用值清單時）。
...
* 的長度 **字串** 欄位一律應使用欄定義。 依預設，Adobe Campaign中的長度上限為255，但Adobe建議，如果您已知道大小不會超過較短的長度，請讓欄位保持較短。
...
除了 **autopk** 在大多數的表格中預設會定義，您應該考慮新增一些邏輯或業務金鑰（帳號、使用者端號碼等）。 稍後可用於匯入/調解或資料套件。 如需詳細資訊，請參閱 [識別碼](#identifiers).
...
| 識別碼 | 說明 | 最佳實務 |
|--- |--- |--- |
| ID | <ul><li>ID是Adobe Campaign表格的實體主要索引鍵。 對於現成可用的表格，這是從序列產生的32位元數字</li><li>此識別碼通常是特定Adobe Campaign執行個體所獨有的。 </li><li>自動產生的ID可顯示在結構描述定義中。 搜尋 *autopk=&quot;true&quot;* 屬性。</li></ul> | <ul><li>自動產生的識別碼不應在工作流程或封裝定義中作為參考使用。</li><li>不應假設ID一律為遞增的數字。</li><li>現成可用表格中的ID是32位元數字，不應變更此型別。 此數字取自區段中相同名稱的「序列」。</li></ul> |
| 名稱（或內部名稱） | <ul><li>此資訊是表格中記錄的唯一識別碼。 此值可以手動更新，通常使用產生的名稱。</li><li>此識別碼在Adobe Campaign的其他執行個體中部署時保留其值，且不應空白。</li></ul> | <ul><li>如果要將物件從一個環境部署至另一個環境，請重新命名Adobe Campaign產生的記錄名稱。</li><li>當物件具有名稱空間屬性(*綱要* 例如)，此通用名稱空間將在所有建立的自訂物件間運用。 部分保留的名稱空間不應使用： *nms*， *xtk*， *nl*， *ncl*， *crm*， *xxl*.</li><li>當物件沒有任何名稱空間時(*工作流程* 或 *傳遞* 例如)，此名稱空間概念會新增為內部名稱物件的前置詞： *namespaceMyObjectName*.</li><li>請勿使用空格「 」、分號「：」或連字型大小「 — 」等特殊字元。 所有這些字元都會取代為底線「_」（允許的字元）。 例如，「abc-def」和「abc：def」會儲存為「abc_def」並互相覆寫。</li></ul> |
...
建立自訂表格時，您有兩個選項：
* 自動產生的索引鍵(id)和內部索引鍵（自訂）的組合。 如果您的系統索引鍵是複合索引鍵或非整數，此選項會很有趣。 整數在大表格中及與其他表格結合時，可提供更高的效能。
* 使用主鍵作為外部系統主鍵。 此解決方案通常為較佳選擇，因為它可簡化匯入和匯出資料的方法，並在不同系統間使用一致的金鑰。 如果索引鍵名為「id」，且必須填入外部值（而非自動產生），則應停用Autopk。
...
Adobe Campaign主索引鍵是所有現成可用表格自動產生的id，且自訂表格可以相同。 如需詳細資訊，請參閱[本節](#identifiers)。

此值擷取自所謂的 **序列**，用來產生編號規則的資料庫物件。

序列有兩種型別：
* **已共用**：多個表格會從相同序列中挑選其id。 這表示如果一個資料表使用ID &#39;X&#39;，其他共用相同序列的資料表都不會有該ID &#39;X&#39;的記錄。 **XtkNewId** 是Adobe Campaign中可用的預設共用序列。
...
因此，客戶每年傳送60億封電子郵件，其記錄檔的保留期為180天，4個月內就會用完id。 為避免此類挑戰，請確保根據您的磁碟區有清除設定。 如需詳細資訊，請參閱[本節](#data-retention)。
...
* 索引使用方式會與您的存取模式繫結。 最佳化索引通常是資料庫設計的關鍵部分，必須由專家處理。 新增索引通常是附加至資料庫維護的重複工作流程。 這項工作會隨著時間推移逐步進行，以解決發生時的效能問題。
* 索引會增加整體資料表大小（以儲存索引本身）。
* 在資料行上新增索引可以改善資料讀取存取(SELECT)的效能，但可能會降低資料寫入存取(UPDATE)的效能。
* 由於這會影響資料插入期間的效能，因此應該限制索引的大小和數量。
* 必要時不要新增索引。 確定此為必要專案，並提升查詢的整體效能（測試與學習）。
* 一般而言，如果您知道查詢不會傳回超過10%的記錄，索引會很有效率。
* 請仔細選取需要定義的索引。
* 請勿從現成可用的表格中移除原生索引。
...
1. 以滑鼠右鍵按一下 **[!UICONTROL First name]** 欄位。
1. 選取 **[!UICONTROL Filter on this field]**。

   ![](assets/data-model-index-example.png)

1. 對以下專案重複此操作： **[!UICONTROL Last name]** 欄位。

熒幕上方會新增兩個對應的篩選器。

![](assets/data-model-index-search.png)

您現在可以在以下位置執行搜尋篩選： **[!UICONTROL First name]** 和 **[!UICONTROL Last name]** 根據各種篩選條件篩選欄位。
...
* 如果您想要保留更長的記錄時間，應根據資料庫大小和傳送的訊息數量，謹慎做出此決定。 提醒您，Adobe Campaign序列是32位元整數。
* 建議不要在這些表格中一次擁有超過10億筆記錄（大約佔可用的21.4億id的50%），以限制使用所有可用id的風險。 有些客戶需要將保留期間降低到180天以下。
...
* 此 *產品* 和 *儲存* 表格較小：小於10,000。
* 產品標籤和參考已放在 *產品* 表格。
* 此 *異動料號* 表格只有連結至 *產品* 表格，數值。
