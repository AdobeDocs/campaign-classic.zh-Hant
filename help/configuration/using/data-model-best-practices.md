...
請閱讀本 [檔案](../../configuration/using/about-schema-reference.md) ，以開始使用促銷活動結構。 瞭解如何設定擴充結構，以擴充本檔案中Adobe Campaign資料庫的概念性資料 [模型](../../configuration/using/about-schema-edition.md)。
...
Adobe Campaign預設資料模型會呈現在本 [檔案中](../../configuration/using/data-model-description.md)。
...
除了在大多數 **表中預設定義的autopk** ，您還應考慮添加一些邏輯或業務密鑰（帳戶號、客戶機號等）。 它稍後可用於導入／協調或資料包。 For more on this, see [Identifiers](#identifiers).
...
Adobe Campaign主要金鑰是自動產生的ID，適用於所有現成可用的表格，而自訂表格則相同。 如需詳細資訊，請參閱[本區段](#identifiers)。
...
因此，每年傳送60億封電子郵件，其記錄的保留期為180天，但4個月內就會用完ID。 為避免此類挑戰，請務必根據捲進行清除設定。 如需詳細資訊，請參閱[本區段](#data-retention)。
...
**相關主題：**
* 如需序列自動 **產生功能的詳細資訊** ，請參 [閱本檔案](https://helpx.adobe.com/tw/campaign/kb/sequence_auto_generation.html)。
* 如需序列耗盡的詳細資訊，請觀看 [此影片](https://helpx.adobe.com/customer-care-office-hours/campaign/sequences-exhaustion-campaign-classic.html)。
...
1. 轉到列出資料庫中所有收件人的資料夾。 For more on this, see [Managing profiles](../../platform/using/managing-profiles.md).
