...
Forms是實體 `xtk:form` 型別。 您可以在下列位置檢視輸入表單結構： `xtk:form` 綱要。 若要檢視此結構描述，請選擇 **[!UICONTROL Administration]** > **[!UICONTROL Configuration]** > **[!UICONTROL Data schemas]** 功能表中。 深入瞭解 [表單結構](form-structure.md).
...
若要尋找影像，請選擇 **[!UICONTROL Administration]** > **[!UICONTROL Configuration]** > **[!UICONTROL Images]** 功能表中。
...
1. 插入表單元素。
   ...
   例如，若要插入輸入欄位，請使用 `<input>` 元素。 設定 `xpath` XPath運算式形式之欄位參考的屬性。 [顯示全文](schema-structure.md#referencing-with-xpath)。
...
您可以使用JavaScript SOAP呼叫，從主控台驗證表單資料。 使用這些呼叫進行複雜驗證，例如，根據授權值清單檢查值。 [顯示全文](form-structure.md#soap-methods)。
...
1. 在表單中，新增 `<soapCall>` 元素至 `<leave>` 元素。
