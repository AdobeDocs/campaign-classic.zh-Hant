...
此表與&#x200B;**nms:recipient**&#x200B;方案匹配。

此為傳遞&#x200B;**收件者**&#x200B;的預設表格。 因此，其中包含透過各種管道傳送所需的資訊：
...
如需「收件者」表格的詳細資訊，請參閱[此區段](../../configuration/using/about-data-model.md#default-recipient-table)。

### NmsGroup {#NmsGroup}

此表與&#x200B;**nms:group**&#x200B;方案匹配。

它可讓您建立&#x200B;**收件者**&#x200B;的靜態群組。 收件者和群組之間有多對多關係。 例如，一個收件者可以屬於數個群組，而一個群組可以包含數個收件者。 群組可以透過匯入或傳遞定位手動建立。 群組通常用作傳送目標。 欄位上有一個唯一索引，代表sName群組的內部名稱。 該組連結到一個資料夾(鍵為iFolderId。 如需詳細資訊，請參閱[XtkFolder](#XtkFolder))。
...
欄位上有一個唯一索引，代表sName服務的內部名稱。 服務連結至資料夾（金鑰為iFolderId）。 如需詳細資訊，請參閱[XtkFolder](#XtkFolder))。 最後， iType欄位指定此服務的傳送通道（電子郵件為0，簡訊為1，電話為2，直接郵件為3，傳真為4）。

### NmsSubscription {#NmsSubscription}

此表與&#x200B;**nms:subscription**&#x200B;方案匹配。

它使您能夠管理資訊服務的收件者訂閱。

### NmsSubHisto {#NmsSubHisto}

此表與&#x200B;**nms:subHisto**&#x200B;方案匹配。

如果使用Web表單或應用程式的介面管理訂閱，則所有訂閱和取消訂閱都將在NmsSubHisto表中進行歷史記錄。 iAction欄位指定在儲存在tsDate欄位中的日期上執行的操作（取消訂閱為0，訂閱為1）。

### NmsDelivery {#NmsDelivery}

此表與&#x200B;**nms:delivery**&#x200B;方案匹配。

此表中的每個記錄代表&#x200B;**傳送動作**&#x200B;或&#x200B;**傳送範本**。 它包含執行傳送所需的所有必要參數（目標、內容等）。 在分析階段期間會建立傳送（廣播）日誌(NmsBroadLog)和相關的追蹤URL(NmsTrackingUrl)（請參閱下方，以取得這兩個表格的詳細資訊）。

欄位上有一個唯一索引，代表sInternalName傳送或藍本的內部名稱。 傳遞連結至執行資料夾(外鍵為iFolderProcessId。 如需詳細資訊，請參閱[XtkFolder](#XtkFolder))。

### XtkFolder {#XtkFolder}

它包含控制台的&#x200B;**導航**&#x200B;頁簽中顯示的&#x200B;**樹**&#x200B;中的所有資料夾。
...
* **Nms操作**:此表與nms: **operationschema** 相符。它包含行銷活動的資料。
* **NmsDeliveryOutline**:此表格與 **nms:** deliveryOutlineschema相符。它包含傳送（傳送大綱）的延伸屬性。
* **NmsDlvOutlineItem**:此表與nms: **** dlvOutlineItemschema匹配。它包含傳遞大綱的文章。
* **NmsDeliveryCustomization**:此表格與 **nms:** deliveryCustomizationschema相符。它包含傳遞的個人化欄位。
* **NmsBudget**:此表與nms: **** budgetschema匹配。它包含促銷活動、計畫、方案、任務和/或傳送的預算資料。
* **NmsDocument**:此表與nms: **** documentschema匹配。它以檔案（影像、excel或word檔案等）的形式包含促銷活動的行銷檔案
* **XtkWorkflow**:此表格符合 **xtk:** workflowschema。它包含促銷活動目標定位。
...
* **NmsTypology規則**:此表格符合 **nms:** typologyRuleschema。其中包含根據類型套用至傳送的規則。
* **Nms類型**:此表格符合 **nms:** 類型綱要。其中包含要套用至符合類型之傳送的規則集。
* **NmsTypologyRuleRel**:此表格符合 **nms:** typologyRuleRelschema。它包含類型與其規則之間的關係。
...
此表符合&#x200B;**nms:remaHexposition**&#x200B;方案。 它包含測量假設的定義。
...
* 已聯繫的數量：**iContactRenated**。 假設中目標聯繫的不重複數量。
* 控制組計數：**iProofRenacted**。 假設中目標控制組接觸的不重複數量。
* 聯繫的響應率：**dContactRenactedRate**。 假設中目標接觸的回應率。
* 控制組的響應率：**dProofRenactedRate**。 假設控制組的響應率。
* 聯繫的總人口收入：**dContactRenactedTotalAmount**。 假設中目標聯繫人的總收入。
...
* 每個聯繫人的總利潤：**dContactRenactedTotalMargin**。 假設中定位的每個連絡總餘額。
...
* 重要性：**iAgrivity**（SQL表達式）。 根據促銷活動的重要性，包含0到3的值。

### NmsRemaMatchRcp {#NmsRemaMatchRcp}

此表與&#x200B;**nms:remaMatchRcp**&#x200B;方案匹配。
...
* **Nms模擬**:此表與nms: **模擬方** 案匹配。它代表指定母體上一組傳送或選件的模擬。
...
* **NmsOffer**:此表與nms: **** offerschema相符。其中包含每個行銷優惠方案的定義。
* **NmsPostitionRcp**:此表與nms: **** positionRcpschema相符。它包含傳送給每個人的行銷主張的跨頻道記錄。 記錄是在準備或有效地向個人提出建議時建立的。
* **NmsOfferSpace**:此表格符合 **nms:** offerSpacechema。它包含了提出建議的位置的定義。
* **NmsOfferContext**:此表格符合 **nms:** offerContextschema。它包含關於命題適用性的附加標準以及權重計算公式的定義。
* **NmsOfferView**:此表格與 **nms:offerView**&#x200B;相符。其中包含選件表示法。
...
此表與&#x200B;**nms:rtEvent**&#x200B;架構相符。 其中包含即時事件的定義。

### NmsBatchEvent {#NmsBatchEvent}

![](assets/data-model_message-center_batch.png)

此表與&#x200B;**nms:batchEvent**&#x200B;方案匹配。 它包含依批次的事件定義。
...
* **NmsMobileApp**:此表格符合 **nms:** mobileAppschema。其中包含Adobe Campaign中定義的行動應用程式。
* **NmsAppSubscription**:此表格符合 **nms:** appSubscriptionschema。它包含有關一個或多個應用程式的訂閱者資訊。
* **NmsAppSubscriptionRcp**:此表與nms: **** appSubscriptionRcpschema相符。它可讓您將訂閱應用程式的訪客連結至收件者表格。
* **NmsExcludeLogAppSubRcp**:此表格符合 **nms:** excludeLogAppSubRcpschema。
...
* **NmsVisitor**:此表格與nms: **visitorschema** 相符。它包含訪客的相關資訊。
* **NmsVisitorSub**:此表格符合 **nms:** visitorSubschema。它可讓您將訪客連結至他們訂閱的服務(Twitter或Facebook)。
* **NmsFriendShipRel**:此表與nms: **friendshipRelschema** 相符。它可讓您在Facebook服務的內容中連結訪客與其朋友。
* **NmsVisitorInterestRel**:此表格符合 **nms:** visitorInterestRelschema。它可讓您連結訪客及其興趣。
* **NmsInterest**:此表與nms: **interestschema** 匹配。它包含每個訪客的興趣清單。
