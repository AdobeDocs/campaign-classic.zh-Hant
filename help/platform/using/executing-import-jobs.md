...
product: campaign
title: 設定匯入工作
description: 瞭解如何在Campaign中設定和執行匯入作業
...
>[!NOTE]
>
>您可以使用來匯入資料，而不需將其與資料庫資料對應。 **[!UICONTROL Import a list]** 函式。 然後，資料可以透過以下方式專用於工作流程 **[!UICONTROL Read list]** 物件。 如需詳細資訊，請參閱[此頁面](../../workflow/using/read-list.md)。
...
1. 在中為此匯入輸入一個名稱 **[!UICONTROL Label]** 欄位。 您可以添加描述。
1. 在相應欄位中選擇匯入類型。有兩種可能的匯入型別： **[!UICONTROL Simple import]** 僅匯入一個檔案，以及 **[!UICONTROL Multiple import]** 以在一次執行中匯入多個檔案。

   若為多次匯入，請選取 **[!UICONTROL Multiple import]** 從 **[!UICONTROL Import type]** 匯入精靈第一個畫面中的下拉式清單。

   ![](assets/s_ncs_user_import_wizard01_2.png)

1. 按一下以指定要匯入的欄位 **[!UICONTROL Add]**.

   ![](assets/s_ncs_user_import_wizard01_3.png)

   每次新增檔案時， **[!UICONTROL File to import]** 精靈隨即顯示。 請參閱區段 [步驟2 — 來源檔案選擇](#step-2---source-file-selection) 並依照精靈中的步驟，將匯入選項定義為簡單匯入。
...
依預設， **[!UICONTROL Upload file on the server]** 「 」已選取。 按一下右側的資料夾 **[!UICONTROL Local file]** 欄位以瀏覽本機磁碟並選取要匯入的檔案。 您可以取消選擇此選項以輸入存取路徑以及要在伺服器上匯入的檔案名稱。
...
* **[!UICONTROL Update on server...]** 可讓您將本機檔案傳輸至伺服器。 此選項僅在以下情況下可用： **[!UICONTROL Upload file on the server]** 「 」已選取。
* **[!UICONTROL Download]** 只有在檔案已上傳至伺服器上時，才可使用。
...
按一下 **[!UICONTROL OK]** 若要儲存格式，然後按一下 **[!UICONTROL Next]** 以顯示下一個步驟。
...
* 此 **[!UICONTROL Destination schema]** 欄位可讓您選取要匯入資料的結構描述。 此資訊是強制性的。按一下 **[!UICONTROL Select link]** 圖示以選取其中一個現有的結構描述。 按一下 **[!UICONTROL Edit link]** 以顯示所選表格的內容。
* 中央表顯示源檔中定義的所有欄位。選擇要匯入的欄位以將目標檔案與它們相關聯。可以手動或自動對應這些欄位。
   ...
   若要自動關聯來源欄位和目的地欄位，請按一下 **[!UICONTROL Guess the destination fields]** 圖示填入欄位清單右側。 如果需要，可以修改建議的欄位。
...
* **[!UICONTROL Update (empty) if source value is empty]**：在更新時，如果來源檔案中的欄位為空，則欄位中的值將移除資料庫值。 否則，保留資料庫欄位。
...
* 在中複製管理 **[!UICONTROL Ignore]** 模式或 **[!UICONTROL Reject entity]** 會從匯入中排除重複專案。 在這種情況下，不會匯入任何記錄。
* 在 **[!UICONTROL Reject entity]** 模式)下，不會匯入元素，且匯入記錄檔中會產生錯誤。
* 在 **[!UICONTROL Ignore]** 模式)下，不會匯入元素，但不會保留任何錯誤痕跡。 此模式可讓您優化性能。

>[!IMPORTANT]
>
>重複資料刪除僅在內存中執行。因此，具有重複資料刪除的匯入的大小是有限的。限制取決於幾個參數 (應用程序伺服器的容量，活動，金鑰中的欄位數等)。重複資料刪除的最大大小為 1,000,000 行。

重複資料刪除涉及源檔和資料庫中都存在的記錄。這僅涉及與更新有關的操作(即 **[!UICONTROL Update and insertion]** 或 **[!UICONTROL Update]**)。 此 **[!UICONTROL Duplicate management]** 選項可讓您更新或忽略記錄（如果記錄同時存在於來源檔案和資料庫中）。 此 **[!UICONTROL Update or insert based on origin]** 選項屬於選用模組，且無法在標準內容中使用。

選項 **[!UICONTROL Reject]** 和 **[!UICONTROL Ignore]** 如上所述操作。

### 發生錯誤時 {#behavior-in-the-event-of-an-error}

大部分的資料傳輸操作會產生各種型別的錯誤（不一致的行格式、無效的電子郵件地址等）。 匯入引擎生成的所有錯誤和所有警告都將儲存並連結到匯入執行個體。

![](assets/s_ncs_user_import_general_tab.png)

欲瞭解這些拒絕的詳情，請前往 **[!UICONTROL Rejects]** 標籤。

![](assets/s_ncs_user_import_rejets_tab.png)

拒絕有兩種型別(型別會顯示在 **[!UICONTROL Connector]** column)：
...
* **[!UICONTROL Ignore and log a warning]**：所有欄位都會匯入資料庫，但產生錯誤的欄位除外。
...
* 按一下 **[!UICONTROL Edit]** 連結以選取您要與收件者建立關聯或訂閱的資料夾、清單或服務。
   ...
   1. 與清單相關聯
      ...
      此 **[!UICONTROL Edit...]** 中的連結 **[!UICONTROL Add recipients to a list]** 區段可讓您選取或建立收件者將匯入到的清單。

      ![](assets/s_ncs_user_import_wizard05_5.png)

      您可以按一下「 」，為這些收件者建立新清單 **[!UICONTROL Select link]**，則 **[!UICONTROL Create]**. 清單的建立和管理會顯示在 [本節](../../platform/using/creating-and-managing-lists.md).
...
* 使用 **[!UICONTROL Origin]** 欄位，可將收件者的來源相關資訊新增至其設定檔。 此資訊在多重匯入的框架內特別有用。
