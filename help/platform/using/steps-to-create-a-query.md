...
>[!NOTE]
>
>所有這些步驟都可在通用查詢編輯器中使用。 在另一個內容中建立查詢時，可能會省略一些步驟。\
>「查詢」活動顯示於 [本節](../../workflow/using/query.md).
...
例如，選取 **[!UICONTROL Age]**， **[!UICONTROL Primary key]**， **[!UICONTROL Email domain]** 和 **[!UICONTROL City]**. 結果將根據此選擇進行整理。 使用視窗右側的藍色箭頭來變更欄順序。

![](assets/query_editor_nveau_01.png)

您可以透過將公式插入運算式中或對彙總函式執行程式來編輯運算式。 若要這麼做，請按一下 **[!UICONTROL Expression]** 欄欄位，然後選取 **[!UICONTROL Edit expression]**.

![](assets/query_editor_nveau_97.png)

您可以將輸出欄資料分組：若要這麼做，請核取 **[!UICONTROL Yes]** 在 **[!UICONTROL Group]** 的欄 **[!UICONTROL Data to extract]** 視窗。 此函式產生核取之群組軸周圍的結果。 具有分組的查詢範例可在以下位置取得： [本節](../../workflow/using/querying-delivery-information.md).

![](assets/query_editor_nveau_56.png)

* 此 **[!UICONTROL Handle groupings (GROUP BY + HAVING)]** 函式可讓您「分組依據」並選取已分組的專案（「具有」）。 此函式適用於輸出欄中的所有欄位。 例如，此選項可讓您將輸出欄的所有選擇分組，並復原特定型別的資訊，例如介於35到50之間的收件者。

   如需詳細資訊，請參閱[本章節](../../workflow/using/querying-using-grouping-management.md)。

* 此 **[!UICONTROL Remove duplicate rows (DISTINCT)]** 函式可讓您去除重複在輸出欄中取得的相同結果。 例如，如果您選取輸出欄中的「姓氏」、「名字」和「電子郵件」欄位來進行人口普查，則擁有相同資料的欄位將會被刪除，因為這意味著在資料庫中已經多次輸入了相同連絡人：只會考慮一個結果。

## 步驟3 — 排序資料 {#step-3---sort-data}

此 **[!UICONTROL Sorting]** 視窗可讓您排序欄內容。 使用箭頭來變更欄順序：

* 此 **[!UICONTROL Sorting]** 欄可啟用簡單排序，並按A到Z或升序排列欄內容。
* 此 **[!UICONTROL Descending sort]** 將內容從Z排列到A，並依遞減順序排列。 這對於檢視紀錄銷售非常有用，例如：最高數字會顯示在清單頂端。
...
* 若要建立新篩選器，請選取建立要驗證的公式所需的欄位、運運算元和值，以選取資料。 可合併多個條件(如需詳細資訊，請參閱 [定義篩選條件](../../platform/using/defining-filter-conditions.md))。
* 若要使用先前儲存的篩選器，請按一下 **[!UICONTROL Add]** 按鈕，按一下 **[!UICONTROL Predefined filter]** 並選取您想要的。

   ![](assets/query_editor_15.png)

* 在中建立的篩選器 **[!UICONTROL Generic query editor]** 也可用於其他查詢應用程式，反之亦然。 若要儲存篩選器，請按一下 **[!UICONTROL Save]** 圖示。
...
按一下 **[!UICONTROL Preview]** 標籤來檢視篩選條件的結果。 在此情況下，所有說英語的收件者都會顯示其姓名、名字和電子郵件地址。
...
設定限制篩選條件後，您將會存取 **[!UICONTROL Data formatting]** 視窗。 此視窗可讓您重新排列輸出欄、轉換資料，以及變更欄標籤的大寫/小寫。 它也可讓您使用計算欄位將公式套用至最終結果。
