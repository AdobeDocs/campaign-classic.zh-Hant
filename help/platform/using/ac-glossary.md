...
進一步瞭解 [本地核准活動](../../workflow/using/local-approval.md).
+++
...
* **明確**：在聯絡人登入頻道介面後即可識別身分。
* **隱含**：連絡人已由Cookie識別（永久或工作階段）。 可將它處理為匿名或識別的連絡人。
* **匿名**：無法識別連絡人。
...
此 **行銷資源管理(RM)** Adobe Campaign中的模組可讓您透過提供相關任務、預算和行銷資源的完整管理和即時追蹤，以合作模式控制行銷動作。 Adobe Campaign操作者可透過完整的驗證程式和適當的追蹤工具（報告、追蹤核准、通知、討論論壇等），協調其動作並核准所有階段的進度。
...
* **已識別的連絡人**：已在頻道上自願識別的聯絡人。 在對外互動中，會自動識別聯絡人。
...
選件 **設計環境** 是運運算元建立優惠方案、定義型別規則，以及選取優惠方案鎖定目標的結構描述的環境。 用來儲存產生之優惠方案主張的表格也由環境定義。 預設情況下，互動附加元件會提供 **設計** 環境和 **即時** 連結的環境。 這兩個環境都已預先設定為鎖定內建的收件者表格。
...
優惠方案即時環境已連結至行銷活動 **設計環境**. 此變數包含唯讀優惠方案，其內容和資格已透過 **設計環境**. 您可以選取它們以在網站上顯示，或插入至外寄訊息。
...
進一步瞭解 [浮動視窗](../../workflow/using/building-a-workflow.md#adding-and-linking-activities).
+++
...
個人化變數是傳遞中的程式碼片段，可根據收件者的資訊向不同收件者顯示不同文字。 這些欄位可以個人化欄位或區塊的形式實作。
...
推播通知是行動應用程式收到的訊息。 推播通知的設定是透過在行動應用程式中包含Experience PlatformSDK程式碼來與Adobe Campaign搭配使用。 推播可提供兩個傳送頻道：iOS和Android。
...
進一步瞭解 [綱要延伸](../../configuration/using/extending-a-schema.md).
+++
...
進一步瞭解 [已觸發的行銷活動](../../workflow/using/external-signal.md).
+++
